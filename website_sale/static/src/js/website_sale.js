        ...
        if (salePage.dataset.ecomZoomAuto) {
            ...
            for (const image of images) {
                ...
                this.zoomCleanup.push(() => {
                    ...
                    const zoomemdad = $image.data("zoomemdad");
                    if (zoomemdad) {
                        zoomemdad.hide();
                        $image.unbind();
                    }
        ...
        if (salePage.dataset.ecomZoomClick) {
            ...
            for (const image of images ) {
                const handler = () => {
                    if (salePage.dataset.ecomZoomAuto) {
                        // Remove any flyout
                        const flyouts = document.querySelectorAll(".zoomemdad-flyout");
...
publicWidget.registry.WebsiteSaleLayout = publicWidget.Widget.extend({
    ...
    _onApplyShopLayoutChange: function (ev) {
        ...
        if (wysiwyg) {
            wysiwyg.emdadEditor.observerUnactive('_onApplyShopLayoutChange');
        }
        ...
        if (wysiwyg) {
            wysiwyg.emdadEditor.observerActive('_onApplyShopLayoutChange');
        }
    },
});
