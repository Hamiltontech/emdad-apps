...
from emdad.tools.pdf import emdadPdfFileReader, emdadPdfFileWriter
    ...
    def _render_qweb_pdf_prepare_streams(self, report_ref, data, res_ids=None):
        ...
        if report.report_name in ('account.report_invoice_with_payments', 'account.report_invoice'):
            ...
            if qr_inv_ids:
                ...
                if header:
                    ...
                    for invoice_id, stream in qr_res.items():
                        qr_pdf = emdadPdfFileReader(stream['stream'], strict=False)
                        header_pdf = emdadPdfFileReader(header_res[invoice_id]['stream'], strict=False)

                        page = header_pdf.getPage(0)
                        page.mergePage(qr_pdf.getPage(0))

                        output_pdf = emdadPdfFileWriter()
            ...
            for invoice_id, additional_stream in streams_to_append.items():
                ...
                writer = emdadPdfFileWriter()
                writer.appendPagesFromReader(emdadPdfFileReader(invoice_stream, strict=False))
                writer.appendPagesFromReader(emdadPdfFileReader(additional_stream['stream'], strict=False))
                new_pdf_stream = io.BytesIO()
