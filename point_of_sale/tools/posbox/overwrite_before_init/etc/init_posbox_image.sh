...
# This will be modified by a unique password on the first start of emdad
...
wget 'https://nightly.emdad.com/master/iotbox/eftdvs' -P /usr/local/bin/
chmod +x /usr/local/bin/eftdvs
wget 'https://nightly.emdad.com/master/iotbox/eftapi.so' -P /usr/lib/

groupadd usbusers
usermod -a -G usbusers pi
usermod -a -G lp pi
usermod -a -G input lightdm
mkdir -v /var/log/emdad
chown pi:pi /var/log/emdad
chown pi:pi -R /home/pi/emdad/
...
echo "* * * * * rm /var/run/emdad/sessions/*" | crontab -
...
echo "addons/hw_drivers/iot_devices/" > /home/pi/emdad/.git/info/exclude
