...
REPO=https://github.com/emdad/emdad.git
...
CLONE_DIR="${OVERWRITE_FILES_BEFORE_INIT_DIR}/home/pi/emdad"
...
emdad/
emdad-bin" | tee --append .git/info/sparse-checkout > /dev/null
...
# copy iotbox version
mkdir -pv "${MOUNT_POINT}"/var/emdad
echo "${VERSION_IOTBOX}" | tee "${MOUNT_POINT}"/var/emdad/iotbox_version "${MOUNT_POINT}"/home/pi/iotbox_version
