...
echo "addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/emdad" >> .git/info/sparse-checkout

git fetch "${localremote}" "${localbranch}" --depth=1
git reset "${localremote}"/"${localbranch}" --hard

git clean -dfx
cp -a /home/pi/emdad/addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/emdad/* /home/pi/emdad/
rm -r /home/pi/emdad/addons/point_of_sale/tools/posbox/overwrite_after_init
...
(sleep 5 && sudo service emdad restart) &
