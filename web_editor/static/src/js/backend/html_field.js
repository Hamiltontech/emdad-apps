...
import {
    ...
    getRangePosition
} from '@web_editor/js/editor/emdad-editor/src/utils/utils';
...
export class HtmlField extends Component {
    ...
    _filterPowerBoxCommands(commands) {
        ...
        if (this.wysiwyg && this.wysiwyg.emdadEditor) {
            const selection = this.wysiwyg.emdadEditor.document.getSelection();
    ...
    async startWysiwyg(wysiwyg) {
        ...
        wysiwyg.$editable[0].classList.add("emdad-editor-qweb");
        ...
        this.wysiwyg.emdadEditor.addEventListener("historyStep", () =>
    ...
    toggleCodeView() {
        ...
        if (this.wysiwyg) {
            this.wysiwyg.emdadEditor.observerUnactive('toggleCodeView');
            if (this.state.showCodeView) {
                this.wysiwyg.$editable.remove();
                this.wysiwyg.emdadEditor.toolbarHide();
                const value = this.wysiwyg.getValue();
                this.props.record.update({ [this.props.name]: value });
            } else {
                this.wysiwyg.emdadEditor.observerActive('toggleCodeView');
            }
        }
        if (!this.state.showCodeView) {
    ...
    onDynamicPlaceholderValidate(chain, defaultValue) {
        if (chain) {
            ...
            this.wysiwyg.emdadEditor.execCommand('insert', t);
            // Ensure the dynamic placeholder <t> element is sanitized.
            this.wysiwyg.emdadEditor.sanitize(t);
        }
    }
    ...
    async commitChanges({ urgent } = {}) {
        if (this._isDirty() || urgent) {
            ...
            if (this.wysiwyg && this.wysiwyg.emdadEditor) {
                this.wysiwyg.emdadEditor.observerUnactive('commitChanges');
                ...
                this.wysiwyg.emdadEditor.observerActive('commitChanges');
            }
    ...
    async _lazyloadWysiwyg() {
        ...
        let wysiwygModule = await emdad.loader.modules.get('@web_editor/js/wysiwyg/wysiwyg');
        this.MoveNodePlugin = (await emdad.loader.modules.get('@web_editor/js/wysiwyg/MoveNodePlugin'))?.MoveNodePlugin;
        // Otherwise, load the module.
        if (!wysiwygModule) {
            await loadBundle('web_editor.backend_assets_wysiwyg');
            wysiwygModule = await emdad.loader.modules.get('@web_editor/js/wysiwyg/wysiwyg');
            this.MoveNodePlugin = (await emdad.loader.modules.get('@web_editor/js/wysiwyg/MoveNodePlugin')).MoveNodePlugin;
        }
    ...
    async _toInline() {
        ...
        this.wysiwyg.emdadEditor.sanitize(this.wysiwyg.emdadEditor.editable);
        const html = this.wysiwyg.getValue();
        const $emdadEditor = $editable.closest('.emdad-editor-editable');
        // Save correct nodes references.
        // Remove temporarily the class so that css editing will not be converted.
        $emdadEditor.removeClass('emdad-editor-editable');
        $editable.html(html);

        await toInline($editable, this.cssRules, this.wysiwyg.$iframe);
        $emdadEditor.addClass('emdad-editor-editable');
...
export const htmlField = {
    ...
    extractProps({ attrs, options }, dynamicInfo) {
        ...
        return {
            codeview: Boolean(emdad.debug && options.codeview),
            placeholder: attrs.placeholder,
