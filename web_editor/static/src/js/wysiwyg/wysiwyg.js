...
import * as emdadEditorLib from "@web_editor/js/editor/emdad-editor/src/emdadEditor";
...
import { isSelectionInSelectors, peek } from '@web_editor/js/editor/emdad-editor/src/utils/utils';
...
const emdadEditor = emdadEditorLib.emdadEditor;
const getDeepRange = emdadEditorLib.getDeepRange;
const getInSelection = emdadEditorLib.getInSelection;
const isProtected = emdadEditorLib.isProtected;
const rgbToHex = emdadEditorLib.rgbToHex;
const preserveCursor = emdadEditorLib.preserveCursor;
const closestElement = emdadEditorLib.closestElement;
const setSelection = emdadEditorLib.setSelection;
const endPos = emdadEditorLib.endPos;
const hasValidSelection = emdadEditorLib.hasValidSelection;
const parseHTML = emdadEditorLib.parseHTML;
const closestBlock = emdadEditorLib.closestBlock;
const getRangePosition = emdadEditorLib.getRangePosition;
...
export class Wysiwyg extends Component {
    ...
    setup() {
        ...
        const getColorPickedHandler = (colorType) => {
            return (params) => {
                if (this.hadNonCollapsedSelectionBeforeColorpicker) {
                    this.emdadEditor.historyResetLatestComputedSelection(true);
                }
                // Unstash the mutations now that the color is picked.
                this.emdadEditor.historyUnstash();
                ...
                // hides it).
                if (this.emdadEditor.deselectTable() && hasValidSelection(this.emdadEditor.editable)) {
                    this.emdadEditor.document.getSelection().collapseToStart();
                }
        ...
        const getColorHoverHandler = (colorType) => {
            return (props) => {
                ...
                this.emdadEditor.historyPauseSteps();
                try {
                    this._processAndApplyColor(colorType, props.color, true);
                } finally {
                    this.emdadEditor.historyUnpauseSteps();
                }
            }
        };
        ...
        const colorPaletteCommonOptions = {
            ...
            withGradients: true,
            onColorLeave: () => {
                this.emdadEditor.historyRevertCurrentStep();
            },
            onInputEnter: (ev) => {
    ...
    async startEdition() {
        ...
        this.emdadEditor = new emdadEditor(this.$editable[0], Object.assign({
            _t: _t,
        ...
        this.emdadEditor.addEventListener('contentChanged', function () {
        ...
        if (this.emdadEditor.document !== document) {
            this.emdadEditor.document.addEventListener("mousemove", this._signalOnline, true);
            this.emdadEditor.document.addEventListener("keydown", this._signalOnline, true);
            this.emdadEditor.document.addEventListener("keyup", this._signalOnline, true);
        }

        this._initialValue = this.getValue();
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length) {
            $wrapwrap[0].addEventListener('scroll', this.emdadEditor.multiselectionRefresh, { passive: true });
        ...
        this._observeemdadFieldChanges();
        this.$editable.on(
            'mousedown touchstart',
            '[data-oe-field]',
            function () {
                self.emdadEditor.observerUnactive();
                const $field = $(this);
                ...
                self.emdadEditor.observerActive();
            }
        );
        ...
        this.$editable.on('dblclick', mediaSelector, ev => {
            ...
            if (isEditable) {
                ...
                if (!isProtected(this.emdadEditor.document.getSelection().anchorNode)) {
        ...
        if (options.snippets) {
            $(this.emdadEditor.document.body).addClass('editor_enable');
        ...
        if (this.options.getContentEditableAreas) {
            $(this.options.getContentEditableAreas(this.emdadEditor)).find('*').off('mousedown mouseup click');
        }
        ...
        $(this.emdadEditor.editable).on('mouseup', this._updateEditorUI.bind(this));
        $(this.emdadEditor.editable).on('keydown', this._updateEditorUI.bind(this));
        $(this.emdadEditor.editable).on('keydown', this._handleShortcuts.bind(this));
        ...
        this.emdadEditor.document.addEventListener('selectionchange', this._onSelectionChange);
        this.setCSSVariables(this.snippetsMenu ? this.snippetsMenu.el : this.toolbarEl);

        this.emdadEditor.addEventListener('preObserverActive', () => {
            ...
            this._onPostSanitize(this.emdadEditor.editable);
        });

        if (this.options.autohideToolbar) {
            if (this.emdadEditor.isMobile) {
                this.emdadEditor.editable.before(this.toolbarEl);
            } else {
    ...
    setupToolbar(toolbarEl) {
        ...
        this.emdadEditor.setupToolbar(toolbarEl);
    ...
    destroy() {
        ...
        if (this.emdadEditor) {
            this.emdadEditor.document.removeEventListener("mousemove", this._signalOnline, true);
            this.emdadEditor.document.removeEventListener("keydown", this._signalOnline, true);
            this.emdadEditor.document.removeEventListener("keyup", this._signalOnline, true);
            this.emdadEditor.document.removeEventListener('selectionchange', this._onSelectionChange);
            this.emdadEditor.destroy();
        }
        if (this.snippetsMenu) {
        ...
        const $wrapwrap = $('#wrapwrap');
        if ($wrapwrap.length && this.emdadEditor) {
            $('#wrapwrap')[0].removeEventListener('scroll', this.emdadEditor.multiselectionRefresh, { passive: true });
        }
        $(this.$root).off('click');
    ...
    _renderElement() {
        ...
        if (this.options.resizable && !isMobileOS()) {
            const $wrapper = $('<div class="o_wysiwyg_wrapper emdad-editor">');
            this.$root = $wrapper;
    ...
    getValue(options) {
        ...
        this.emdadEditor && this.emdadEditor.cleanForSave($editable[0]);
    ...
    async saveContent(reload = true, editable = false) {
        ...
        if (!editable) {
            ...
            const editables = "getContentEditableAreas" in this.options ? this.options.getContentEditableAreas(this.emdadEditor) : [];
     ...
     */
    setValue(value) {
        this.emdadEditor.resetContent(value);
    }
    /**
     * Undo one step of change in the editor.
     */
    undo() {
        this.emdadEditor.historyUndo();
    }
    /**
     * Redo one step of change in the editor.
     */
    redo() {
        this.emdadEditor.historyRedo();
    }
    /**
     ...
     */
    focus() {
        if (this.emdadEditor && !this.emdadEditor.historyResetLatestComputedSelection(true)) {
    ...
    getDeepRange() {
        return getDeepRange(this.emdadEditor.editable);
    }
    closestElement(...args) {
        return closestElement(...args);
    }
    async cleanForSave(editable = this.emdadEditor.editable) {
        if (this.emdadEditor) {
            this.emdadEditor.cleanForSave(editable);
    ...
    isSelectionInEditable() {
        return this.emdadEditor.isSelectionInEditable();
    }
    /**
     * Start or resume the emdad field changes muation observers.
     *
     * Necessary to keep all copies of a given field at the same value throughout the page.
     */
    _observeemdadFieldChanges() {
        ...
        if (this.emdadFieldObservers) {
            for (let observerData of this.emdadFieldObservers) {
                observerData.observer.observe(observerData.field, observerOptions);
            }
        } else {
            const emdadFieldSelector = '[data-oe-model], [data-oe-translation-initial-sha]';
            const $emdadFields = this.$editable.find(emdadFieldSelector);
            this.emdadFieldObservers = [];

            $emdadFields.each((i, field) => {
                const observer = new MutationObserver((mutations) => {
                    mutations = this.emdadEditor.filterMutationRecords(mutations);
                    ...
                    let $nodes = $emdadFields.filter(function () {
                        return this !== field;
                    });
                    ...
                    this._pauseemdadFieldObservers();
                    ...
                    this.emdadEditor.withoutRollback(() => {
                ...
                this.emdadFieldObservers.push({field: field, observer: observer});
            });
        }
    }
    /**
    ...
    openLinkToolsFromSelection() {
        const targetEl = this.emdadEditor.document.getSelection().getRangeAt(0).startContainer;
        ...
        if (targetEl.nodeType === Node.ELEMENT_NODE
            ...
            this.emdadEditor.dispatchEvent(new Event('activate_image_link_tool'));
            return;
        }
        this.toggleLinkTools();
    }
    ...
    async toggleLinkTools(options = {}) {
        ...
        const linkEl = getInSelection(this.emdadEditor.document, 'a');
        ...
        if (this.snippetsMenu && !options.forceDialog) {
            ...
            if (options.forceOpen || !this.state.linkToolProps) {
                ...
                if (!this.state.linkToolProps || ![options.link, ...wysiwygUtils.ancestors(options.link)].includes(this.linkToolsInfos.link)) {
                    const { link } = getOrCreateLink({
                        containerNode: this.emdadEditor.editable,
                    ...
                    const addHintClasses = () => {
                        this.emdadEditor.observerUnactive("hint_classes");
                        link.classList.add('oe_edited_link');
                        $button.addClass('active');
                        this.emdadEditor.observerActive("hint_classes");
                    };
                    ...
                    this.state.linkToolProps = {
                        ...
                        wysiwyg: this,
                        editable: this.emdadEditor.editable,
                        link,
                ...
                const _onClick = ev => {
                    if (
                        ...
                        this.destroyLinkTools();
                        this.emdadEditor.document.removeEventListener('click', _onClick, true);
                        document.removeEventListener('click', _onClick, true);
                    }
                };
                this.emdadEditor.document.addEventListener('click', _onClick, true);
                document.addEventListener('click', _onClick, true);
            } else {
                this.destroyLinkTools();
            }
        } else {
            const historyStepIndex = this.emdadEditor.historySize() - 1;
            this.emdadEditor.historyPauseSteps();
            ...
            this.env.services.dialog.add(LinkDialog, {
                ...
                onClose: () => {
                    this.emdadEditor.historyUnpauseSteps();
                    this.emdadEditor.historyRevertUntil(historyStepIndex)
                }
            });
        }
    }
    ...
    openChatGPTDialog(mode = 'prompt') {
        const restore = preserveCursor(this.emdadEditor.document);
        const params = {
            insert: content => {
                this.emdadEditor.historyPauseSteps();
                const insertedNodes = this.emdadEditor.execCommand('insert', content);
                this.emdadEditor.historyUnpauseSteps();
                ...
                if (start && end) {
                    const divContainer = this.emdadEditor.editable.parentElement;
        ...
        if (mode === 'alternatives') {
            params.originalText = this.emdadEditor.document.getSelection().toString() || '';
        }
        this.emdadEditor.document.getSelection().collapseToEnd();
    ...
    removeLink() {
        if (this.snippetsMenu && wysiwygUtils.isImg(this.lastElement)) {
            this.snippetsMenu._mutex.exec(() => {
                // Wait for the editor panel to be fully updated.
                this.emdadEditor.dispatchEvent(new Event('deactivate_image_link_tool'));
            });
        } else {
            this.emdadEditor.execCommand('unlink');
        }
    }
    /**
    ...
    showImageFullscreen(url) {
        ...
        this.emdadEditor.document.getSelection()?.collapseToEnd();
        this.emdadEditor.editable.blur();
    }
    /**
    ...
    openMediaDialog(params = {}) {
        const sel = this.emdadEditor.document.getSelection();
        ...
        const restoreSelection = preserveCursor(this.emdadEditor.document);

        const editable = emdadEditorLib.closestElement(params.node || range.startContainer, '.o_editable') || this.emdadEditor.editable;
    ...
    // todo: test me
    showEmojiPicker() {
        const targetEl = this.emdadEditor.document.getSelection();
        ...
        const cursorAt = getRangePosition(targetEl, this.emdadEditor.document).left;
        let position = 'end';
        ...
        this.popover.add(closest, EmojiPicker, {
                onSelect: (str) => {
                    restoreSelection();
                    this.emdadEditor.execCommand('insert', str);
                },
            },
    ...
    _onMediaDialogSave(params, element) {
        ...
        if (params.node) {
            ...
            this.emdadEditor.unbreakableStepUnactive();
            this.emdadEditor.historyStep();
        } else {
            return this.emdadEditor.execCommand('insert', element);
        }
    ...
    getInSelection(selector) {
        return getInSelection(this.emdadEditor.document, selector);
    }
    /**
    ...
    async _createSnippetsMenuInstance(options={}) {
        const snippetsEditor = await emdad.loader.modules.get('@web_editor/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    _configureToolbar(options) {
        ...
        const openTools = e => {
            ...
            switch (e.currentTarget.id) {
                ...
                case 'open-chatgpt': {
                    this.openChatGPTDialog(this.emdadEditor.document.getSelection()?.isCollapsed ? 'prompt' : 'alternatives');
                    break;
                }
            }
        };
        ...
        $toolbar.find('#image-crop').click(e => {
            ...
            this.emdadEditor.toolbarHide();
            $(this.lastMediaClicked).on('image_cropper_destroyed', () => this.emdadEditor.toolbarShow());
        });
        $toolbar.find('#image-transform').click(e => {
            ...
            $image.transfo({document: this.emdadEditor.document});
            const mouseup = () => {
                $('#image-transform').toggleClass('active', $image.is('[style*="transform"]'));
            };
            $(this.emdadEditor.document).on('mouseup', mouseup);
            const mousedown = mousedownEvent => {
                if (!$(mousedownEvent.target).closest('.transfo-container').length) {
                    $image.transfo('destroy');
                    $(this.emdadEditor.document).off('mousedown', mousedown).off('mouseup', mouseup);
                }
            ...
            $(this.emdadEditor.document).on('mousedown', mousedown);
        });
    ...
    _processAndApplyColor(colorType, color, previewMode) {
        ...
        let coloredElements = this.emdadEditor.execCommand('applyColor', color, colorType === 'text' ? 'color' : 'backgroundColor', this.lastMediaClicked);
        // Some nodes returned by applyColor can be removed of the document by the sanitization in historyStep
        coloredElements = coloredElements.filter(element => this.emdadEditor.document.contains(element));
        ...
        this.emdadEditor.updateColorpickerLabels({
     ...
     */
    _updateEditorUI(e) {
        let selection = this.emdadEditor.document.getSelection();
        if (!selection) return;
        ...
        const editorWindow = this.emdadEditor.document.defaultView;
        ...
        if (isInMedia) {
            ...
            const selection = this.emdadEditor.document.getSelection();
            const range = this.emdadEditor.document.createRange();
        ...
        this.emdadEditor.updateToolbarPosition();
    ...
    _updateMediaJustifyButton(commandState) {
        ...
        // Change the ID to prevent emdadEditor from controlling it as this is
    ...
    _getBannerCommand(title, alertClass, iconClass, description, priority) {
        return {
            ...
            isDisabled: () => isSelectionInSelectors('.o_editor_banner') || !this.emdadEditor.isSelectionInBlockRoot(),
            callback: () => {
                const bannerElement = parseHTML(this.emdadEditor.document, `
                ...
                `).childNodes[0];
                this.emdadEditor.execCommand('insert', bannerElement);
    ...
    _getPowerboxOptions() {
        ...
        const commands = [
            ...
            this._getBannerCommand(_t('Banner Danger'), 'danger', 'fa-exclamation-circle', _t('Insert a danger banner'), 21),
            {
                ...
                callback: () => {
                    this.emdadEditor.execCommand('setTag', 'blockquote');
                },
            },
            {
                category: _t('Structure'),
                ...
                isDisabled: () => !this.emdadEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.emdadEditor.execCommand('setTag', 'pre');
                },
            },
            {
                ...
                callback: async () => {
                    ...
                    if (user && user.signature) {
                        this.emdadEditor.execCommand('insert', parseHTML(this.emdadEditor.document, user.signature));
                    }
                },
            },
            {
            ...
            commands.push(
                {
                    ...
                    callback: () => this.emdadEditor.execCommand('columnize', 2, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.emdadEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                        const anchor = this.emdadEditor.document.getSelection().anchorNode;
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.emdadEditor.execCommand('columnize', 3, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.emdadEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.emdadEditor.execCommand('columnize', 4, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.emdadEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.emdadEditor.execCommand('columnize', 0),
                    isDisabled: () => {
                        if (!this.emdadEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
    ...
    async _saveElement($el, context) {
        ...
        // remove ZeroWidthSpace from emdad field value
        // ZeroWidthSpace may be present from emdadEditor edition process
    ...
    _onSelectionChange() {
        if (this.emdadEditor.autohideToolbar && this.linkPopover) {
            const selectionInLink = getInSelection(this.emdadEditor.document, 'a') === this.linkPopover.target;
    ...
    async _signalOnline() {
        ...
        this.ptp.notifyAllClients('oe_history_step', peek(this.emdadEditor.historyGetSteps()), { transport: 'rtc' });
    }
    /**
    ...
    async _processMissingSteps(missingSteps) {
        ...
        this.ptp && this.emdadEditor.onExternalHistorySteps(missingSteps);
        return true;
    }
    _showConflictDialog() {
        if (this._conflictDialogOpened) return;
        const content = markup(this.emdadEditor.editable.cloneNode(true).outerHTML);
    ...
    _getNewPtp() {
        ...
        return new PeerToPeer({
            ...
            onRequest: {
                ...
                get_missing_steps: (params) => this.emdadEditor.historyGetMissingSteps(params.requestPayload),
                get_history_from_snapshot: () => this._getHistorySnapshot(),
                get_collaborative_selection: () => this.emdadEditor.getCurrentCollaborativeSelection(),
                recover_document: (params) => {
                    const { serverDocumentId, fromStepId } = params.requestPayload;
                    if (!this.emdadEditor.historyGetBranchIds().includes(serverDocumentId)) {
                        return;
                    }
                    return {
                        missingSteps: this.emdadEditor.historyGetMissingSteps({ fromStepId }),
            ...
            onNotification: async ({ fromClientId, notificationName, notificationPayload }) => {
                switch (notificationName) {
                    case 'ptp_remove':
                        this.emdadEditor.multiselectionRemove(notificationPayload);
                        break;
                    case 'ptp_disconnect':
                        this.ptp.removeClient(fromClientId);
                        this.emdadEditor.multiselectionRemove(fromClientId);
                        break;
                    case 'rtc_data_channel_open': {
                        ...
                        const getClientNamePromise = this.requestClient(
                            ...
                            this.emdadEditor.multiselectionRefresh();
                        });
                    ...
                    case 'oe_history_step':
                        if (this._historySyncFinished) {
                            this.emdadEditor.onExternalHistorySteps([notificationPayload]);
                        } else {
                    ...
                    case 'oe_history_set_selection': {
                        ...
                        this.emdadEditor.onExternalMultiselectionUpdate(selection);
                        break;
                    }
                }
            }
    ...
    async _setCollaborativeSelection(fromClientId) {
        ...
        if (remoteSelection) {
            this.emdadEditor.onExternalMultiselectionUpdate(remoteSelection);
        }
    }
    /**
    ...
    _isLastDocumentStale() {
        ...
        return !this.emdadEditor.historyGetBranchIds().includes(this._serverLastStepId);
    }
    /**
            ...
            for (const peerId of allPeers) {
                this.requestClient(
                    ...
                    'recover_document', {
                        ...
                        fromStepId: peek(this.emdadEditor.historyGetBranchIds()),
                    },
                    { transport: 'rtc' }
                ).then((response) => {
                    nbPendingResponses--;
    ...
    async _resetFromServerAndResyncWithClients() {
        ...
        // lastHistoryId will be different if the emdad bus did not have time to
        // notify the user.
    ...
    async _resetFromClient(fromClientId, resetCollabCount) {
        ...
        if (this._historyStepsBuffer.length) {
            this.emdadEditor.onExternalHistorySteps(this._historyStepsBuffer);
            this._historyStepsBuffer = [];
        }
    ...
    async resetValue(value) {
        ...
        if (this._serverLastStepId) {
            this.emdadEditor.historySetInitialId(this._serverLastStepId);
        }
    }
    /**
    ...
    async resetEditor(value, options) {
        ...
        this._currentClientId = this._generateClientId();
        this.emdadEditor.collaborationSetClientId(this._currentClientId);
        this.resetValue(value);
    ...
    _getHistorySnapshot() {
        return Object.assign(
            {},
            this.emdadEditor.historyGetSnapshotSteps(),
    ...
    _applySnapshot(snapshot) {
        ...
        this._historySyncAtLeastOnce = true;
        this.emdadEditor.historyResetFromSteps(steps, historyIds);
        this.emdadEditor.historyResetLatestComputedSelection();
        return true;
    }
    /**
    ...
    _onPostSanitize(node) {
        ...
        // solution is merged.
        if (node?.querySelectorAll && this.emdadEditor && !this.emdadEditor._fixLinkMutatedElements) {
    ...
    _attachHistoryIds(editable = this.emdadEditor.editable) {
        if (this.options.collaborative) {
            ...
            const historyIds = this.emdadEditor.historyGetBranchIds().join(',');
