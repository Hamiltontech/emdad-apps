...
const resetOuids = emdadEditorLib.resetOuids;
let _serviceCache = {
    orm: {},
    rpc: {},
};
...
const SnippetOptionWidget = Widget.extend({
    ...
    _select: async function (previewMode, widget) {
        ...
        if (previewMode === true) {
            this.options.wysiwyg.emdadEditor.automaticStepUnactive('preview_option');
        }
        ...
        if (previewMode === 'reset' || previewMode === false) {
            this.options.wysiwyg.emdadEditor.automaticStepActive('preview_option');
        }
    ...
    start: function () {
        ...
        this.$handles.on('mousedown', function (ev) {
            ...
            if (rowEl.classList.contains("o_grid_mode") && !isMobile) {
                self.options.wysiwyg.emdadEditor.observerUnactive('displayBackgroundGrid');
                backgroundGridEl = gridUtils._addBackgroundGrid(rowEl, 0);
                self.options.wysiwyg.emdadEditor.observerActive('displayBackgroundGrid');
            ...
            self.options.wysiwyg.emdadEditor.automaticStepUnactive('resizing');
            ...
            const bodyMouseUp = function () {
                ...
                self.options.wysiwyg.emdadEditor.automaticStepActive('resizing');
                ...
                setTimeout(function () {
                    self.options.wysiwyg.emdadEditor.historyStep();
...
registry.layout_column = SnippetOptionWidget.extend(ColumnLayoutMixin, {
    ...
    async selectStyle(previewMode, widgetValue, params) {
        ...
        if (["row-gap", "column-gap"].includes(params.cssProperty) && !isMobileView) {
            ...
            this.options.wysiwyg.emdadEditor.observerUnactive("addGridPreview");
            ...
            this.options.wysiwyg.emdadEditor.observerActive("addGridPreview");
     ...
     */
    _removeGridPreview() {
        this.options.wysiwyg.emdadEditor.observerUnactive("removeGridPreview");
        ...
        this.options.wysiwyg.emdadEditor.observerActive("removeGridPreview");
    },
    /**
     * @returns {boolean}
     */
...
registry.GridColumns = SnippetOptionWidget.extend({
    ...
    async selectStyle(previewMode, widgetValue, params) {
        ...
        if (["--grid-item-padding-y", "--grid-item-padding-x"].includes(params.cssProperty)) {
            ...
            this.options.wysiwyg.emdadEditor.observerUnactive("addPaddingPreview");
            this.$target[0].classList.add("o_we_padding_highlight");
            this.options.wysiwyg.emdadEditor.observerActive("addPaddingPreview");
    ...
    _removePaddingPreview() {
        this.options.wysiwyg.emdadEditor.observerUnactive("removePaddingPreview");
        ...
        this.options.wysiwyg.emdadEditor.observerActive("removePaddingPreview");
    },
});
...
registry.ReplaceMedia = SnippetOptionWidget.extend({
     ...
     */
    onFocus() {
        this.options.wysiwyg.emdadEditor.addEventListener('activate_image_link_tool', this._activateLinkTool);
        this.options.wysiwyg.emdadEditor.addEventListener('deactivate_image_link_tool', this._deactivateLinkTool);
     ...
     */
    onBlur() {
        this.options.wysiwyg.emdadEditor.removeEventListener('activate_image_link_tool', this._activateLinkTool);
        this.options.wysiwyg.emdadEditor.removeEventListener('deactivate_image_link_tool', this._deactivateLinkTool);
    },
...
￼    */
    ...
    _computeMaxDisplayWidth() {
        ...
        // width since we only use col-lg-* in emdad).
...
registry.ColoredLevelBackground = registry.BackgroundToggler.extend({
    ...
    _markColorLevel: function () {
        this.options.wysiwyg.emdadEditor.observerUnactive('_markColorLevel');
        this.$target.addClass('o_colored_level');
        this.options.wysiwyg.emdadEditor.observerActive('_markColorLevel');
    },
});
...
registry.VersionControl = SnippetOptionWidget.extend({
    ...
    async replaceSnippet() {
        ...
        this.options.wysiwyg.waitForEmptyMutexAction().then(async () => {
            ...
            this.options.wysiwyg.emdadEditor.historyUnpauseSteps();
