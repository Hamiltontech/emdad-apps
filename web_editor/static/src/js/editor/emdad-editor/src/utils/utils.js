...
export function closestElement(node, predicate = "*") {
    ...
    return element?.closest('.emdad-editor-editable') && element;
}
...
export function getNormalizedCursorPosition(node, offset, full = true) {
    const editable = closestElement(node, '.emdad-editor-editable');
...
// TODO rename this function in master: it also handles emdad icons, not only
...
export function prepareUpdate(...args) {
    ...
    while (positions.length) {
        ...
        if (options.debug) {
            const editable = el && closestElement(el, '.emdad-editor-editable');
