...
import { closestElement, isUnremovable } from "@web_editor/js/editor/emdad-editor/src/utils/utils";
...
var SnippetEditor = Widget.extend({
    ...
    _outPreviousDropzone(self, currentDropzoneEl) {
        ...
        if (rowEl.classList.contains('o_grid_mode')) {
            ...
            self.options.wysiwyg.emdadEditor.observerActive('dragAndDropMoveSnippet');
            gridUtils._resizeGrid(rowEl);
            self.options.wysiwyg.emdadEditor.observerUnactive('dragAndDropMoveSnippet');
    ...
    _onDragAndDropStart({ helper, addStyle }) {
        this.options.wysiwyg.emdadEditor.observerUnactive('dragAndDropMoveSnippet');
        this.trigger_up('drag_and_drop_start');
        this.options.wysiwyg.emdadEditor.automaticStepUnactive();
        var self = this;
        this.dragState = {};
        ...
        if (rowEl.classList.contains('row') && this.options.isWebsite) {
            if (allowGridMode) {
                ...
                if (!rowEl.classList.contains('o_grid_mode')) {
                    this.options.wysiwyg.emdadEditor.observerActive('dragAndDropMoveSnippet');
    ...
    _onDragAndDropStop({ x, y }) {
        this.options.wysiwyg.emdadEditor.automaticStepActive();
        this.options.wysiwyg.emdadEditor.automaticStepSkipStack();
        this.options.wysiwyg.emdadEditor.unbreakableStepUnactive();
...
var SnippetsMenu = Widget.extend({
    ...
    async start() {
        ...
        $(this.options.wysiwyg.emdadEditor.document.body).on('click', this._checkEditorToolbarVisibilityCallback);
        ...
        this.options.wysiwyg.emdadEditor.addEventListener('historyUndo', refreshSnippetEditors);
        this.options.wysiwyg.emdadEditor.addEventListener('historyRedo', refreshSnippetEditors);
        ...
        return Promise.all(defs).then(() => {
            ...
            if ($undoButton.length) {
                const updateHistoryButtons = () => {
                    $undoButton.attr('disabled', !this.options.wysiwyg.emdadEditor.historyCanUndo());
                    $redoButton.attr('disabled', !this.options.wysiwyg.emdadEditor.historyCanRedo());
                };
                this.options.wysiwyg.emdadEditor.addEventListener('historyStep', updateHistoryButtons);
                this.options.wysiwyg.emdadEditor.addEventListener('observerApply', () => {
                    $(this.options.wysiwyg.emdadEditor.editable).trigger('content_changed');
                });
            }
    ...
    _makeSnippetDraggable() {
        ...
        const dragAndDropOptions = this.options.getDragAndDropOptions({
            ...
            onDragStart: ({ element }) => {
                ...
                const doc = this.options.wysiwyg.emdadEditor.document;
                $(doc.body).addClass('oe_dropzone_active');

                this.options.wysiwyg.emdadEditor.automaticStepUnactive();

                this.$el.find('.oe_snippet_thumbnail').addClass('o_we_already_dragging');
                this.options.wysiwyg.emdadEditor.observerUnactive('dragAndDropCreateSnippet');
            ...
            onDragEnd: async ({ x, y, helper }) => {
                ...
                this.options.wysiwyg.emdadEditor.observerActive('dragAndDropCreateSnippet');
   ...
   _onSnippetDragAndDropStart: function () {
    ...
    _onSnippetClick() {
        ...
        for (const el of $els) {
            if (!el.children.length) {
                $(el).emdadBounce('o_we_snippet_area_animation');
            }
        }
    },
    /**
    ...
    _addToolbar(toolbarMode = "text") {
        ...
        this.options.wysiwyg.emdadEditor.bindExecCommand(customizeTableBlock);
    ...
    _checkEditorToolbarVisibility: function (e) {
        ...
        const selection = this.options.wysiwyg.emdadEditor.document.getSelection();
    ...
    _onRequestEditable: function (ev) {
        ev.data.callback($(this.options.wysiwyg.emdadEditor.editable));
    },
    /**
