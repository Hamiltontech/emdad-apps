/** @emdad-module **/
import { setSelection } from "@web_editor/js/editor/emdad-editor/src/utils/utils";
...
function onMount() {
    const editor = wysiwyg.emdadEditor;
...
QUnit.module(
    ...
    function () {
        ...
        QUnit.test("typing valid URL + space should convert to link", async function (assert) {
                ...
                const node = editable.querySelector("p");
                setSelection(node, 0);
                inputText(".emdad-editor-editable p", "http://google.co.in");
                insertText(editor, " ");
                editor.clean();
                assert.strictEqual(
                    editable.innerHTML,
        ...
        QUnit.test("typing invalid URL + space should not convert to link", async function (assert) {
            ...
            const node = editable.querySelector("p");
            setSelection(node, 0);
            inputText(".emdad-editor-editable p", "www.emdad");
            insertText(editor, " ");
            editor.clean();
            assert.strictEqual(editable.innerHTML, "<p>www.emdad </p>");
        });
