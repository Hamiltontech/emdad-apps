...
import "@website/libs/zoomemdad/zoomemdad";
...
export const WebsiteRoot = publicRootData.PublicRoot.extend({
    ...
    start: function () {
        ...
        this.$('.zoomable img[data-zoom]').zoomemdad();
    ...
    async _loadGMapAPI(editableMode, refetch) {
        ...
        if (refetch || !this._gmapAPILoading) {
            this._gmapAPILoading = new Promise(async resolve => {
                ...
                window.emdad_gmap_api_post_load = (async function emdad_gmap_api_post_load() {
                ...
                await loadJS(`https://maps.googleapis.com/maps/api/js?v=3.exp&libraries=places&callback=emdad_gmap_api_post_load&key=${encodeURIComponent(key)}`);
            });
        }
