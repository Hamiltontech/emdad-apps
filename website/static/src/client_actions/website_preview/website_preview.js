...
export class WebsitePreview extends Component {
    setup() {
        ...
        onMounted(() => {
            ...
            // emdadFrameContentLoaded event to unblock the iframe, as it is
            // triggered faster than the load event.
            this.iframe.el.addEventListener('emdadFrameContentLoaded', () => this.websiteService.unblockPreview('load-iframe'), { once: true });
        });
        ...
        /**
         * This removes the 'emdad' prefix of the title service to display
        ...
        useEffect(() => {
            this.iframe.el.addEventListener('emdadFrameContentLoaded', toggleIsMobile);
            return () => this.iframe.el.removeEventListener('emdadFrameContentLoaded', toggleIsMobile);
        }, () => []);
    }
    ...
    reloadIframe(url) {
        return new Promise((resolve, reject) => {
            this.websiteService.websiteRootInstance = undefined;
            this.iframe.el.addEventListener('emdadFrameContentLoaded', resolve, { once: true });
            if (url) {
