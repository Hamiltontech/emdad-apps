...
from emdad.http import root, Request, Response, SessionExpiredException, get_default_session
from emdad.modules.registry import Registry
from emdad.service import model as service_model
from emdad.service.server import CommonServer
from emdad.service.security import check_session
...
def acquire_cursor(db):
    ...
    for tryno in range(1, MAX_TRY_ON_POOL_ERROR + 1):
        with suppress(PoolError):
            return emdad.registry(db).cursor()
...
class Websocket:
    ...
    def __init__(self, sock, session):
        ...
        self._selector = (
            ...
            if emdad.evented and hasattr(selectors, 'PollSelector')
...
class WebsocketConnectionHandler:
    ...
    def _handle_public_configuration(cls, request):
        if not os.getenv('emdad_BUS_PUBLIC_SAMESITE_WS'):
            return
