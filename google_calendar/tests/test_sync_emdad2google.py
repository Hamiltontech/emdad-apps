...
from emdad.addons.google_account.models.google_service import GoogleService
from emdad.addons.google_calendar.models.res_users import User
from emdad.addons.google_calendar.tests.test_sync_common import TestSyncGoogle, patch_api
from emdad.tests.common import users, warmup
from emdad.tests import tagged
from emdad import tools

@tagged('emdad2google')
@patch.object(User, '_get_google_calendar_token', lambda user: 'dummy-token')
class TestSyncemdad2Google(TestSyncGoogle):
    ...
    def test_event_creation(self):
        ...
        event._sync_emdad2google(self.google_service)
    ...
    def test_event_creation_perf(self):
        ...
        with self.assertQueryCount(__system__=615):
            ...
            events._sync_emdad2google(self.google_service)
    ...
    def test_event_allday_creation(self):
        ...
        self.assertGoogleEventInserted({
            ...
            'organizer': {'email': 'emdadbot@example.com', 'self': True},
            'attendees': [{'email': 'emdadbot@example.com', 'responseStatus': 'accepted'}],
            'extendedProperties': {'shared': {'%s_emdad_id' % self.env.cr.dbname: event.id}}
        })
    ...
    def test_recurrence(self):
        ...
        recurrence._sync_emdad2google(self.google_service)
        self.assertGoogleEventInserted({
            'id': False,
            'start': {'date': '2020-01-15'},
            'end': {'date': '2020-01-16'},
            ...
            'extendedProperties': {'shared': {'%s_emdad_id' % self.env.cr.dbname: recurrence.id}}
        })
    ...
    def test_event_added_to_recurrence(self):
        ...
        self.assertGoogleEventInserted({
            ...
            'attendees': [{'email': 'emdadbot@example.com', 'responseStatus': 'accepted'}],
            'recurrence': ['RRULE:FREQ=WEEKLY;COUNT=2;BYDAY=WE'],
            'extendedProperties': {'shared': {'%s_emdad_id' % self.env.cr.dbname: event.recurrence_id.id}}
        }, timeout=3)
    ...
    def test_all_event_updated(self):
        ...
        self.assertGoogleEventInserted({
            ...
            'extendedProperties': {'shared': {'%s_emdad_id' % self.env.cr.dbname: new_recurrence.id}},
            'reminders': {'overrides': [], 'useDefault': False},
            'visibility': 'public',
        }, timeout=3)
    ...
    def test_send_update_do_request(self, mock_do_request):
        ...
        event.with_context(send_updates=True)._sync_emdad2google(self.google_service)
    ...
    def test_not_send_update_do_request(self, mock_do_request):
        ...
        event.with_context(send_updates=False)._sync_emdad2google(self.google_service)
        self.call_post_commit_hooks()
    ...
    def test_recurrence_delete_single_events(self):
        ...
        self.assertGoogleEventPatched(event_1.google_id, {
            ...
            'organizer': {'email': 'emdadbot@example.com', 'self': True},
            'attendees': [{'email': 'emdadbot@example.com', 'responseStatus': 'accepted'}],
            'extendedProperties': {'shared': {'%s_emdad_id' % self.env.cr.dbname: event_1.id}},
            'reminders': {'overrides': [], 'useDefault': False},
            'visibility': 'public',
            'status': 'cancelled'
        }, timeout=3)
    ...
    def test_create_event_with_sync_config_paused(self):
        ...
        record._sync_emdad2google(self.google_service)
    ...
    def test_update_synced_event_with_sync_config_paused(self):
        ...
        self.assertFalse(self.env.user.google_synchronization_stopped)
        self.assertEqual(self.env.user._get_google_sync_status(), "sync_paused")
        self.assertEqual(record.name, "Updated Event", "Assert that event name was updated in emdad Calendar")
        self.assertTrue(record.need_sync, "Sync variable must be true for updating event when sync re-activates")
    ...
    def test_delete_synced_event_with_sync_config_paused(self):
        """
        Deletes a synced event with synchronization paused, event must be archived in emdad and
        ...
        # Assert that synchronization is paused, delete wasn't called and record was archived in emdad.
        self.assertFalse(self.env.user.google_synchronization_stopped)
        self.assertEqual(self.env.user._get_google_sync_status(), "sync_paused")
        self.assertFalse(record.active, "Event must be archived in emdad after unlinking it")
