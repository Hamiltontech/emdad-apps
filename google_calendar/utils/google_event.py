...
from emdad.tools import email_normalize, ReadonlyDict
import logging
...
class GoogleEvent(abc.Set):
    ...
    Inspired by emdad recordset, one instance can be a single Google event or a
    ...
    def emdad_id(self, env):
        self.emdad_ids(env)  # load ids
        return self._emdad_id

    def _meta_emdad_id(self, dbname):
        """Returns the emdad id stored in the Google Event metadata.
        ...
        o_id = properties.get('%s_emdad_id' % dbname)
        if o_id:
            return int(o_id)

    def emdad_ids(self, env):
        ids = tuple(e._emdad_id for e in self if e._emdad_id)
        if len(ids) == len(self):
            return ids
        model = self._get_model(env)
        found = self._load_emdad_ids_from_db(env, model)
        unsure = self - found
        if unsure:
            unsure._load_emdad_ids_from_metadata(env, model)

        return tuple(e._emdad_id for e in self)

    def _load_emdad_ids_from_metadata(self, env, model):
        unsure_emdad_ids = tuple(e._meta_emdad_id(env.cr.dbname) for e in self)
        emdad_events = model.browse(_id for _id in unsure_emdad_ids if _id)

        # Extended properties are copied when splitting a recurrence Google side.
        # Hence, we may have two Google recurrences linked to the same emdad id.
        # Therefore, we only consider emdad records without google id when trying
        # to match events.
        o_ids = emdad_events.exists().filtered(lambda e: not e.google_id).ids
        for e in self:
            emdad_id = e._meta_emdad_id(env.cr.dbname)
            if emdad_id in o_ids:
                e._events[e.id]['_emdad_id'] = emdad_id

    def _load_emdad_ids_from_db(self, env, model):
        emdad_events = model.with_context(active_test=False)._from_google_ids(self.ids)
        mapping = {e.google_id: e.id for e in emdad_events}  # {google_id: emdad_id}
        existing_google_ids = emdad_events.mapped('google_id')
        for e in self:
            emdad_id = mapping.get(e.id)
            if emdad_id:
                e._events[e.id]['_emdad_id'] = emdad_id
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and emdad.
        # Let userA, userB be two new users (never synced to Google before).
        # UserA creates an event in emdad (they are the owner) but userB syncs first.
        ...
        # (they are the organizer in Google). The "real" owner (in emdad) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in emdad several users might have the same email :/
    ...
    def clear_type_ambiguity(self, env):
        ...
        recurrences = ambiguous_events._load_emdad_ids_from_db(env, env['calendar.recurrence'])
    ...
    def exists(self, env) -> 'GoogleEvent':
        ...
        recurrences.emdad_ids(env)
        events.emdad_ids(env)

        return self.filter(lambda e: e._emdad_id)
    ...
    def get_emdad_event(self, env):
        if self._get_model(env)._name == 'calendar.event':
            return env['calendar.event'].browse(self.emdad_id(self.env))
        else:
            return env['calendar.recurrence'].browse(self.emdad_id(self.env)).base_event_id
