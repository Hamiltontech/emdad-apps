...
from emdad import api, fields, models, registry, _
from emdad.tools import ormcache_context, email_normalize
from emdad.exceptions import UserError
from emdad.osv import expression

from emdad.addons.google_calendar.utils.google_event import GoogleEvent
from emdad.addons.google_calendar.utils.google_calendar import GoogleCalendarService
from emdad.addons.google_account.models.google_service import TIMEOUT

_logger = logging.getLogger(__name__)


# API requests are sent to Google Calendar after the current transaction ends.
# This ensures changes are sent to Google only if they really happened in the emdad database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Google if the first creation crashed in emdad.
...
class GoogleSync(models.AbstractModel):
    ...
    def unlink(self):
        ...
        if self.env.context.get('archive_on_error') and self._active_name:
            ...
            # Then, since it has been deleted on Google, the event is also deleted on emdad DB (_sync_google2emdad).
            self.action_archive()
            return True
        return super().unlink()
    ...
    def _sync_emdad2google(self, google_service: GoogleCalendarService):
        if not self:
            return
    ...
    @api.model
    def _sync_google2emdad(self, google_events: GoogleEvent, default_reminders=()):
        """Synchronize Google recurrences in emdad. Creates new recurrences, updates
        existing ones.

        :param google_recurrences: Google recurrences to synchronize in emdad
        :return: synchronized emdad recurrences
        """
        existing = google_events.exists(self.env)
        new = google_events - existing - google_events.cancelled()

        emdad_values = [
            dict(self._emdad_values(e, default_reminders), need_sync=False)
            for e in new
        ]
        new_emdad = self.with_context(dont_notify=True)._create_from_google(new, emdad_values)
        cancelled = existing.cancelled()
        cancelled_emdad = self.browse(cancelled.emdad_ids(self.env))

        # Check if it is a recurring event that has been rescheduled.
        # We have to check if an event already exists in emdad.
        # Explanation:
        ...
        if rescheduled_events:
            ...
            cancelled_emdad += self.env['calendar.event'].search([('google_id', 'in', google_ids_to_remove)])

        cancelled_emdad._cancel()
        synced_records = new_emdad + cancelled_emdad
        for gevent in existing - cancelled:
            # Last updated wins.
            # This could be dangerous if google server time and emdad server time are different
            updated = parse(gevent.updated)
            emdad_record = self.browse(gevent.emdad_id(self.env))
            # Migration from 13.4 does not fill write_date. Therefore, we force the update from Google.
            if not emdad_record.write_date or updated >= pytz.utc.localize(emdad_record.write_date):
                vals = dict(self._emdad_values(gevent, default_reminders), need_sync=False)
                emdad_record.with_context(dont_notify=True)._write_from_google(gevent, vals)
                synced_records |= emdad_record
    ...
    def _get_records_to_sync(self, full_sync=False):
        """Return records that should be synced from emdad to Google
    ...
    @api.model
    def _emdad_values(self, google_event: GoogleEvent, default_reminders=()):
