...
from emdad.addons.google_calendar.utils.google_calendar import GoogleCalendarService, InvalidSyncToken
from emdad.addons.google_calendar.models.google_sync import google_calendar_token
from emdad.loglevels import exception_to_unicode
from emdad.tools import str2bool
...
class User(models.Model):
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        synced_recurrences = self.env['calendar.recurrence']._sync_google2emdad(recurrences)
        synced_events = self.env['calendar.event']._sync_google2emdad(events - recurrences, default_reminders=default_reminders)

        # emdad -> Google
        recurrences = self.env['calendar.recurrence']._get_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences.with_context(send_updates=send_updates)._sync_emdad2google(calendar_service)
        ...
        (events - synced_events).with_context(send_updates=send_updates)._sync_emdad2google(calendar_service)

        return bool(events | synced_events) or bool(recurrences | synced_recurrences)

    def _sync_single_event(self, calendar_service: GoogleCalendarService, emdad_event, event_id):
        self.ensure_one()
        ...
        synced_events = self.env['calendar.event']._sync_google2emdad(event, default_reminders=default_reminders)
        # emdad -> Google
        emdad_event.with_context(send_updates=send_updates)._sync_emdad2google(calendar_service)
        return bool(emdad_event | synced_events)
