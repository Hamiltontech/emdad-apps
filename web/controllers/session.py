...
from emdad.modules import module
from emdad.exceptions import AccessError, UserError, AccessDenied
...
class Session(http.Controller):
    ...
    def authenticate(self, db, login, password, base_location=None):
        ...
        if pre_uid != request.session.uid:
            # Crapy workaround for unupdatable emdad Mobile App iOS (Thanks Apple :@) and Android
        ...
        registry = emdad.modules.registry.Registry(db)
        with registry.cursor() as cr:
            env = emdad.api.Environment(cr, request.session.uid, request.session.context)
    ...
    def account(self):
        ...
        return 'https://accounts.emdad.com/oauth2/auth?' + url_encode(params)
