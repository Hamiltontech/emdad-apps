...
from emdad import SUPERUSER_ID, _, http
from emdad.addons.base.models.assetsbundle import ANY_UNIQUE
from emdad.exceptions import AccessError, UserError
from emdad.http import request, Response
from emdad.tools import file_open, file_path, replace_exceptions
from emdad.tools.image import image_guess_size_from_field_name
from emdad.tools.mimetypes import guess_mimetype

_logger = logging.getLogger(__name__)

BAD_X_SENDFILE_ERROR = """\
emdad is running with --x-sendfile but is receiving /web/filestore requests.

With --x-sendfile enabled, NGINX should be serving the
/web/filestore route, however emdad is receiving the
request.
...
class Binary(http.Controller):
    ...
    def content_filestore(self, _path):
        if emdad.tools.config['x_sendfile']:
            # pylint: disable=logging-format-interpolation
            _logger.error(BAD_X_SENDFILE_ERROR.format(
                data_dir=emdad.tools.config['data_dir']
            ))
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        imgext = '.png'
        dbname = request.db
        uid = (request.session.uid if dbname else None) or emdad.SUPERUSER_ID
        ...
        if not dbname:
            ...
            try:
                ...
                registry = emdad.modules.registry.Registry(dbname)
