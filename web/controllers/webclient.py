...
from emdad.modules import get_manifest
...
@lazy
def CONTENT_MAXAGE():
    warnings.warn("CONTENT_MAXAGE is a deprecated alias to emdad.http.STATIC_CACHE_LONG", DeprecationWarning)
...
class WebClient(http.Controller):
    ...
    def bootstrap_translations(self, mods=None):
        ...
        if mods is None:
            mods = emdad.conf.server_wide_modules or []
            if request.db:
    ...
    def translations(self, unique, mods=None, lang=None):
        ...
        if mods:
            ...
            mods = list(request.env.registry._init_modules) + (emdad.conf.server_wide_modules or [])
    ...
    def version_info(self):
        return emdad.service.common.exp_version()
