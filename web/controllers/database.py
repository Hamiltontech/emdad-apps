...
from emdad.http import content_disposition, dispatch_rpc, request, Response
from emdad.service import db
from emdad.tools.misc import file_open, str2bool
from emdad.tools.translate import _

from emdad.addons.base.models.ir_qweb import render as qweb_render
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = emdad.tools.config.verify_admin_password('admin')
        d['list_db'] = emdad.tools.config['list_db']
        d['langs'] = emdad.service.db.exp_list_lang()
        d['countries'] = emdad.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = emdad.service.db.list_db_incompatible(d['databases'])
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = emdad.tools.config.verify_admin_password('admin')
    ...
    def backup(self, master_pwd, name, backup_format='zip'):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            emdad.service.db.check_super(master_pwd)
            ...
            dump_stream = emdad.service.db.dump_db(name, None, backup_format)
