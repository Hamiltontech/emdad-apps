...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "emdad" });
    assert.strictEqual(env.services.title.current, "emdad");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "emdad", chat: null });
    assert.strictEqual(env.services.title.current, "emdad");
    env.services.title.setParts({ action: "Import" });
    assert.strictEqual(env.services.title.current, "emdad - Import");
});
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "emdad", action: "Import" });
    assert.strictEqual(env.services.title.current, "emdad - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "emdad", action: "Import" });
    parts.action = "Export";
    assert.strictEqual(env.services.title.current, "emdad - Import"); // parts is a copy!
});
