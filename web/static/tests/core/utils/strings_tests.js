...
QUnit.module("utils", () => {
    ...
    QUnit.test("escapeRegExp", (assert) => {
        ...
        assert.deepEqual(escapeRegExp("[wowl]"), "\\[wowl\\]");
        assert.deepEqual(escapeRegExp("[wowl.emdad]"), "\\[wowl\\.emdad\\]");
        assert.deepEqual(
            escapeRegExp("^emdad.define([.]*)$"),
            "\\^emdad\\.define\\(\\[\\.\\]\\*\\)\\$"
        );
        assert.deepEqual(
            escapeRegExp("[.*+?^${}()|[]\\"),
    ...
    QUnit.test("escape", (assert) => {
        ...
        assert.strictEqual(
            escape(`<a href="https://www.emdad.com">emdad<a>`),
            `&lt;a href=&quot;https://www.emdad.com&quot;&gt;emdad&lt;a&gt;`
        );
        assert.strictEqual(
            escape(`<a href='https://www.emdad.com'>emdad<a>`),
            `&lt;a href=&#x27;https://www.emdad.com&#x27;&gt;emdad&lt;a&gt;`
        ...
        assert.strictEqual(
            escape("<a href='https://www.emdad.com'>emdad`s website<a>"),
            `&lt;a href=&#x27;https://www.emdad.com&#x27;&gt;emdad&#x60;s website&lt;a&gt;`
        );
    });
    ...
    QUnit.test("isEmail", (assert) => {
        ...
        assert.notOk(isEmail("test@emdad"));
        assert.notOk(isEmail("test@emdad@emdad.com"));
        assert.notOk(isEmail("te st@emdad.com"));

        assert.ok(isEmail("test@emdad.com"));
    });
});
