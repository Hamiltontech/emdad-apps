...
import { Component, useState, markup } from "@emdad/owl";
...
export const emdadExceptionTitleMap = new Map(
    Object.entries({
        "emdad.addons.base.models.ir_mail_server.MailDeliveryException": _t("MailDeliveryException"),
        "emdad.exceptions.AccessDenied": _t("Access Denied"),
        "emdad.exceptions.MissingError": _t("Missing Record"),
        "emdad.exceptions.UserError": _t("Invalid Operation"),
        "emdad.exceptions.ValidationError": _t("Validation Error"),
        "emdad.exceptions.AccessError": _t("Access Error"),
        "emdad.exceptions.Warning": _t("Warning"),
    })
);
...
ErrorDialog.title = _t("emdad Error");
...
ClientErrorDialog.title = _t("emdad Client Error");

// -----------------------------------------------------------------------------
// Network Error Dialog
// -----------------------------------------------------------------------------
export class NetworkErrorDialog extends ErrorDialog {}
NetworkErrorDialog.title = _t("emdad Network Error");
...
export class RPCErrorDialog extends ErrorDialog {
    ...
    inferTitle() {
        ...
        if (this.props.exceptionName && emdadExceptionTitleMap.has(this.props.exceptionName)) {
            this.title = emdadExceptionTitleMap.get(this.props.exceptionName).toString();
            return;
        }
        ...
        switch (this.props.type) {
            case "server":
                this.title = _t("emdad Server Error");
                break;
            case "script":
                this.title = _t("emdad Client Error");
                break;
            case "network":
                this.title = _t("emdad Network Error");
                break;
        }
    }
...
export class WarningDialog extends Component {
    ...
    inferTitle() {
        if (this.props.exceptionName && emdadExceptionTitleMap.has(this.props.exceptionName)) {
            return emdadExceptionTitleMap.get(this.props.exceptionName).toString();
        }
        return this.props.title || _t("emdad Warning");
    }
}
...
export class RedirectWarningDialog extends Component {
    setup() {
        ...
        this.title = capitalize(subType) || _t("emdad Warning");
        this.message = message;
...
SessionExpiredDialog.title = _t("emdad Session Expired");
SessionExpiredDialog.props = { ...standardErrorDialogProps };

registry
    .category("error_dialogs")
    .add("emdad.exceptions.AccessDenied", WarningDialog)
    .add("emdad.exceptions.AccessError", WarningDialog)
    .add("emdad.exceptions.MissingError", WarningDialog)
    .add("emdad.exceptions.UserError", WarningDialog)
    .add("emdad.exceptions.ValidationError", WarningDialog)
    .add("emdad.exceptions.RedirectWarning", RedirectWarningDialog)
    .add("emdad.http.SessionExpiredException", SessionExpiredDialog)
