...
@emdad.tests.tagged('click_all', 'post_install', '-at_install', '-standard')
class TestMenusAdmin(emdad.tests.HttpCase):
    ...
    def test_01_click_everywhere_as_admin(self):
        ...
        for app_id in menus['root']['children']:
            with self.subTest(app=menus[app_id]['name']):
                ...
                self.browser_js("/web", "emdad.loader.modules.get('@web/webclient/clickbot/clickbot_loader').startClickEverywhere('%s');" % menus[app_id]['xmlid'], "emdad.isReady === true", login="admin", timeout=1200)
...
class TestMenusDemo(HttpCaseWithUserDemo):
    ...
    def test_01_click_everywhere_as_demo(self):
        ...
        for app_id in menus['root']['children']:
            with self.subTest(app=menus[app_id]['name']):
                _logger.runbot('Testing %s', menus[app_id]['name'])
                self.browser_js("/web", "emdad.loader.modules.get('@web/webclient/clickbot/clickbot_loader').startClickEverywhere('%s');" % menus[app_id]['xmlid'], "emdad.isReady === true", login="demo", timeout=1200)

@emdad.tests.tagged('post_install', '-at_install')
class TestMenusAdminLight(emdad.tests.HttpCase):
    allow_end_on_form = True
    def test_01_click_apps_menus_as_admin(self):
        ...
        self.browser_js("/web", "emdad.loader.modules.get('@web/webclient/clickbot/clickbot_loader').startClickEverywhere(undefined, true);", "emdad.isReady === true", login="admin", timeout=120)
...
class TestMenusDemoLight(HttpCaseWithUserDemo):
    ...
    def test_01_click_apps_menus_as_demo(self):
        ...
        self.browser_js("/web", "emdad.loader.modules.get('@web/webclient/clickbot/clickbot_loader').startClickEverywhere(undefined, true);", "emdad.isReady === true", login="demo", timeout=120)
