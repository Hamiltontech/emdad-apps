...
from emdad import fields, http, tools, _
from emdad.addons.http_routing.models.ir_http import slug, unslug
from emdad.addons.website.controllers.main import QueryURL
from emdad.addons.website.models.ir_http import sitemap_qs2dom
from emdad.addons.website_profile.controllers.main import WebsiteProfile
from emdad.exceptions import AccessError, ValidationError, UserError, MissingError
from emdad.http import request, Response
from emdad.osv import expression
from emdad.tools import consteq, email_split
    ...
    def _set_viewed_slide(self, slide, quiz_attempts_inc=False):
        if not slide.channel_id.is_member:
            if not isinstance(request.session.get('viewed_slides'), dict):
                # Compatibility layer with emdad 15.0,
                # where `viewed_slides` are stored as `list` in sessions.
                # For performance concerns, `viewed_slides` is changed to a dict,
                # but sessions coming from emdad 15.0 after an upgrade should still be compatible.
                # This compatibility layer regarding `viewed_slides` must remain from emdad 16.0 and above,
                # as this is possible to do a jump of multiple versions in one go,
                # and carry the sessions with the upgrade.
                # e.g. upgrade from emdad 15.0 to 18.0.
