...
import { getRangePosition } from '@web_editor/js/editor/emdad-editor/src/utils/utils';
import { utils as uiUtils } from "@web/core/ui/ui_service";
import { useSubEnv, status, markup } from "@emdad/owl";
...
export class MassMailingHtmlField extends HtmlField {
    ...
    async commitChanges() {
        ...
        this._pendingCommitChanges = (async () => {
            ...
            this.wysiwyg.emdadEditor.historyPauseSteps();
            ...
            this.wysiwyg.emdadEditor.observerUnactive('toInline');
            ...
            iframe.remove();
            this.wysiwyg.emdadEditor.observerActive('toInline');
            const inlineHtml = editableClone.innerHTML;
            $editorEnable.addClass('editor_enable');
            this.wysiwyg.emdadEditor.historyUnpauseSteps();
            this.wysiwyg.emdadEditor.historyRevertCurrentStep();
    ...
    async _resetIframe() {
        ...
        this.wysiwyg.$iframeBody.find('.emdad-editor-editable').removeClass('o_editable');

        initializeDesignTabCss(this.wysiwyg.getEditable());
        this.wysiwyg.getEditable().find('img').attr('loading', '');

        this.wysiwyg.emdadEditor.observerFlush();
        this.wysiwyg.emdadEditor.historyReset();
    ...
    async _onSnippetsLoaded() {
        ...
        $snippetsSideBar.on('click', '.o_codeview_btn', () => {
            this.wysiwyg.emdadEditor.observerUnactive();
            $codeview.toggleClass('d-none');
            this.wysiwyg.getEditable().toggleClass('d-none');
            this.wysiwyg.emdadEditor.observerActive();

            if ($codeview.hasClass('d-none')) {
                this.wysiwyg.setValue($codeview.val());
                this.wysiwyg.emdadEditor.sanitize();
                this.wysiwyg.emdadEditor.historyStep(true);
            } else {
        ...
        $themeSelectorNew.on('click', '.dropdown-item', async (e) => {
            ...
            // the emdad editor before resetting the history.
            setTimeout(() => {
                this.wysiwyg.historyReset();

                // The selection has been lost when switching theme.
                const document = this.wysiwyg.emdadEditor.document;
                const selection = document.getSelection();
                const p = this.wysiwyg.emdadEditor.editable.querySelector('p');
                if (p && selection) {
    ...
    async _switchThemes(themeParams) {
        ...
        this.wysiwyg.emdadEditor.resetContent($newLayout[0].outerHTML);
    ...
    async _lazyloadWysiwyg() {
        ...
        const wysiwygModule = await emdad.loader.modules.get('@mass_mailing/js/mass_mailing_wysiwyg');
