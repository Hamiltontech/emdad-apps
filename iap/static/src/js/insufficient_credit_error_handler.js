...
import { Component, onWillStart } from "@emdad/owl";
...
class InsufficientCreditDialog extends Component {
    ...
    async onWillStart() {
        ...
        const { isEnterprise } = emdad.info;
        if (errorData.trial && isEnterprise) {
            this.buttonMessage = _t("Start a Trial at emdad");
        } else {
...
function insufficientCreditHandler(env, error, originalError) {
    ...
    if (data && data.name === "emdad.addons.iap.tools.iap_tools.InsufficientCreditError") {
