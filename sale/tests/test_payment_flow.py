...
from emdad.tests import tagged, JsonRpcException
from emdad.tools import mute_logger

from emdad.addons.account_payment.tests.common import AccountPaymentCommon
from emdad.addons.payment.tests.http_common import PaymentHttpCommon
from emdad.addons.portal.controllers.portal import CustomerPortal
from emdad.addons.sale.controllers.portal import PaymentPortal
...
class TestSalePayment(AccountPaymentCommon, SaleCommon, PaymentHttpCommon):
    ...
    def test_11_so_payment_link(self):
        ...
        with patch(
            'emdad.addons.payment.controllers.portal.PaymentPortal'
        ...
        with mute_logger('emdad.addons.payment.models.payment_transaction'):
    ...
    def test_12_so_partial_payment_link(self):
        ...
        tx_sudo._set_done()
        with mute_logger('emdad.addons.sale.models.payment_transaction'):
            tx_sudo._finalize_post_processing()
    ...
    def test_invoice_is_final(self):
        ...
        with mute_logger('emdad.addons.sale.models.payment_transaction'), patch(
            'emdad.addons.sale.models.sale_order.SaleOrder._create_invoices',
    ...
    def test_check_payment_access_token_before_rerouting_flow(self):
        ...
        ), patch('emdad.addons.payment.utils.check_access_token') as check_payment_access_token_mock:
            try:
                payment_portal_controller._get_extra_payment_form_values(
                    sale_order_id=self.sale_order.id, access_token='whatever'
                )
    ...
    def test_transaction_route_rejects_unexpected_kwarg(self):
        ...
        with self.assertRaises(JsonRpcException, msg='emdad.exceptions.ValidationError'):
