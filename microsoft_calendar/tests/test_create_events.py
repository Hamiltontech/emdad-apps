...
from emdad.addons.microsoft_calendar.tests.common import TestCommon, mock_get_token
...
class TestCreateEvents(TestCommon):
    ...
    def test_create_simple_event_without_sync(self, mock_insert):
        """
        A emdad event is created when Outlook sync is not enabled.
        """
    ...
    def test_create_simple_event_without_email(self):
        ...
        with self.assertRaises(ValidationError):
            record._sync_emdad2microsoft()

    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_organizer_calendar(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the emdad organizer calendar.
        """
        ...
        self.assert_emdad_event(new_records, self.expected_emdad_event_from_outlook)
        self.assertEqual(new_records.user_id, self.organizer_user)
        self.assertEqual(new_records.need_sync_m, False)

    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_attendee_calendar_and_organizer_exists_in_emdad(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the emdad attendee calendar.
        There is a emdad user that matches with the organizer email address.
        """
    ...
    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_attendee_calendar_and_organizer_does_not_exist_in_emdad(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the emdad attendee calendar.
        no emdad user that matches with the organizer email address.
        """
        ...
        outlook_event = dict(self.simple_event_from_outlook_attendee, organizer={
            'emailAddress': {'address': "john.doe@emdad.com", 'name': "John Doe"},
        })
        expected_event = dict(self.expected_emdad_event_from_outlook, user_id=False)
        ...
        new_records = (records - existing_records)
        self.assertEqual(len(new_records), 1)
        self.assert_emdad_event(new_records, expected_event)

    @patch.object(MicrosoftCalendarService, 'insert')
    def test_create_recurrent_event_without_sync(self, mock_insert):
        """
        A emdad recurrent event is created when Outlook sync is not enabled.
        """
    ...
    def test_create_recurrent_event_with_sync(self, mock_insert, mock_get_events):
        """
        A emdad recurrent event is created when Outlook sync is enabled.
        """
        if not self.sync_emdad_recurrences_with_outlook_feature():
            return
    ...
    def test_create_recurrent_event_with_sync_by_another_user(self, mock_insert, mock_get_events):
        """
        A emdad recurrent event has been created and synced with Outlook by another user, but nothing
    ...
    def test_create_recurrent_event_from_outlook_organizer_calendar(self, mock_get_events):
        """
        A recurrent event has been created in Outlook and synced in the emdad organizer calendar.
        """
        ...
        self.assert_emdad_recurrence(new_recurrences, self.expected_emdad_recurrency_from_outlook)
        for i, e in enumerate(sorted(new_events, key=lambda e: e.id)):
            self.assert_emdad_event(e, self.expected_emdad_recurrency_events_from_outlook[i])

    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_recurrent_event_from_outlook_attendee_calendar(self, mock_get_events):
        """
        A recurrent event has been created in Outlook and synced in the emdad attendee calendar.
        """
    ...
    def test_forbid_recurrences_creation_synced_outlook_calendar(self, mock_insert):
        """
        Forbids new recurrences creation in emdad due to Outlook spam limitation of updating recurrent events.
        """
    ...
    def test_create_event_with_sync_config_paused(self, mock_insert):
        ...
        # Try to create a simple event in emdad Calendar.
        record = self.env["calendar.event"].with_user(self.organizer_user).create(self.simple_event_values)
        self.call_post_commit_hooks()
        record.invalidate_recordset()
    ...
    def test_create_event_for_another_user(self, mock_insert, mock_get_events):
        """
        Allow the creation of event for another user only if the proposed user have its emdad Calendar synced.
        ...
        # In emdad, this event will be created (behind the screens) by a synced emdad user as attendee (self.attendee_user).
        ...
        # Mock event from Microsoft and sync event with emdad through self.attendee_user (synced user).
        ...
        # Ensure that event was successfully created in emdad (no ACL error was triggered blocking creation).
