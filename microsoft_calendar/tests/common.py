...
from emdad.tests.common import HttpCase
...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        # simple event values to create a emdad event
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for emdad events converted to Outlook events (to be posted through API)
        # -----------------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Outlook events converted to emdad events
        # -----------------------------------------------------------------------------------------

        self.expected_emdad_event_from_outlook = {
            "name": "simple_event",
        ...
        self.expected_emdad_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_emdad_recurrency_events_from_outlook = [
            {
    ...
    def sync_emdad_recurrences_with_outlook_feature(self):
        """
    ...
    def create_events_for_tests(self):
        ...
        # Currently, it is forbidden to create recurrences in emdad. A trick for deactivating the checking
    ...
    def assert_emdad_event(self, emdad_event, expected_values):
        """
        Assert that an emdad event has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        self.assertTrue(expected_values)

        emdad_event_values = emdad_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(emdad_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, emdad_event_values.get(k), msg=f"'{k}' mismatch")

    def assert_emdad_recurrence(self, emdad_recurrence, expected_values):
        """
        Assert that an emdad recurrence has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        emdad_recurrence_values = emdad_recurrence.read(list(expected_values.keys()))[0]

        for k, v in expected_values.items():
            self.assertEqual(v, emdad_recurrence_values.get(k), msg=f"'{k}' mismatch")
