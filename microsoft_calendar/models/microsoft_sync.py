...
from emdad import api, fields, models, registry
from emdad.exceptions import UserError
from emdad.osv import expression

from emdad.addons.microsoft_calendar.utils.microsoft_event import MicrosoftEvent
from emdad.addons.microsoft_calendar.utils.microsoft_calendar import MicrosoftCalendarService
from emdad.addons.microsoft_calendar.utils.event_id_storage import IDS_SEPARATOR, combine_ids, split_ids
from emdad.addons.microsoft_account.models.microsoft_service import TIMEOUT
...
# This ensures changes are sent to Microsoft only if they really happened in the emdad database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Microsoft if the first creation crashed in emdad.
...
class MicrosoftSync(models.AbstractModel):
    ...
    def _sync_emdad2microsoft(self):
        if not self:
            return
    ...
    def _sync_recurrence_microsoft2emdad(self, microsoft_events, new_events=None):
        ...
        for recurrent_master in recurrent_masters:
            new_calendar_recurrence = dict(
                self.env['calendar.recurrence']._microsoft_to_emdad_values(recurrent_master, default_values, with_ids=True),
                need_sync_m=False
            )
            ...
            base_values = dict(
                self.env['calendar.event']._microsoft_to_emdad_values(recurrent_master, default_values, with_ids=True),
                need_sync_m=False
            )
            to_create_values = []
            ...
            for recurrent_event in to_create:
                if recurrent_event.type == 'occurrence':
                    value = self.env['calendar.event']._microsoft_to_emdad_recurrence_values(recurrent_event, base_values)
                else:
                    value = self.env['calendar.event']._microsoft_to_emdad_values(recurrent_event, default_values)
            ...
            new_recurrence_emdad = self.env['calendar.recurrence'].with_context(dont_notify=True).create(new_calendar_recurrence)
            new_recurrence_emdad.base_event_id = new_recurrence_emdad.calendar_event_ids[0] if new_recurrence_emdad.calendar_event_ids else False
            new_recurrence |= new_recurrence_emdad
        ...
        for recurrent_master_id in ms_recurrence_ids:
            ...
            for recurrent_event in to_update:
                if recurrent_event.type == 'occurrence':
                    value = self.env['calendar.event']._microsoft_to_emdad_recurrence_values(
    ...
    def _update_microsoft_recurrence(self, recurrence, events):
        """
        Update emdad events from Outlook recurrence and events.
        """
        ...
        for e in events_to_update:
            if e.type == "exception":
                event_values = self.env['calendar.event']._microsoft_to_emdad_values(e)
            elif e.type == "occurrence":
                event_values = self.env['calendar.event']._microsoft_to_emdad_recurrence_values(e)
            else:
                event_values = None
            ...
            if event_values:
                ...
                emdad_event = self.env['calendar.event'].browse(e.emdad_id(self.env)).exists().with_context(
                    no_mail_to_attendees=True, mail_create_nolog=True
                )
                emdad_event.with_context(dont_notify=True).write(dict(event_values, need_sync_m=False))
                update_events |= emdad_event
    ...
    @api.model
    def _sync_microsoft2emdad(self, microsoft_events: MicrosoftEvent):
        """
        Synchronize Microsoft recurrences in emdad.
        Creates new recurrences, updates existing ones.
        :return: synchronized emdad
        """
        existing = microsoft_events.match_with_emdad_events(self.env)
        ...
        emdad_values = [
            dict(self._microsoft_to_emdad_values(e, with_ids=True), need_sync_m=False)
            for e in (new - new_recurrence)
        ]
        synced_events = self.with_context(dont_notify=True)._create_from_microsoft(new, emdad_values)
        synced_recurrences, updated_events = self._sync_recurrence_microsoft2emdad(existing, new_recurrence)
        ...
        # Get sync lower bound days range for checking if old events must be updated in emdad.
        ...
        for mevent in (existing - cancelled).filter(lambda e: e.lastModifiedDateTime):
            # Last updated wins.
            # This could be dangerous if microsoft server time and emdad server time are different
            if mevent.is_recurrence():
                emdad_event = self.env['calendar.recurrence'].browse(mevent.emdad_id(self.env)).exists()
            else:
                emdad_event = self.browse(mevent.emdad_id(self.env)).exists()

            if emdad_event:
                emdad_event_updated_time = pytz.utc.localize(emdad_event.write_date)
                ...
                if lower_bound_day_range:
                    update_time_diff = ms_event_updated_time - emdad_event_updated_time
                    old_event_update_condition = emdad_event._check_old_event_update_required(int(lower_bound_day_range), update_time_diff)

                if ms_event_updated_time >= emdad_event_updated_time and old_event_update_condition:
                    vals = dict(emdad_event._microsoft_to_emdad_values(mevent), need_sync_m=False)
                    emdad_event.with_context(dont_notify=True)._write_from_microsoft(mevent, vals)

                    if emdad_event._name == 'calendar.recurrence':
                        update_events = emdad_event._update_microsoft_recurrence(mevent, microsoft_events)
                        synced_recurrences |= emdad_event
                        synced_events |= update_events
                    else:
                        synced_events |= emdad_event

        return synced_events, synced_recurrences

    def _check_old_event_update_required(self, lower_bound_day_range, update_time_diff):
        """
        Checks if an old event in emdad should be updated locally. This verification is necessary because
        sometimes events in emdad have the same state in Microsoft and even so they trigger updates locally
    ...
    def _microsoft_delete(self, user_id, event_id, timeout=TIMEOUT):
        """
        Once the event has been really removed from the emdad database, remove it from the Outlook calendar.
    ...
    def _microsoft_patch(self, user_id, event_id, values, timeout=TIMEOUT):
        """
        Once the event has been really modified in the emdad database, modify it in the Outlook calendar.
    ...
    def _microsoft_insert(self, values, timeout=TIMEOUT):
        """
        Once the event has been really added in the emdad database, add it in the Outlook calendar.
    ...
    def _microsoft_attendee_answer(self, answer, params, timeout=TIMEOUT):
        ...
        with microsoft_calendar_token(self.env.user.sudo()) as token:
            if token:
                ...
                # former differs for each attendee. This info is required for sending the event answer and emdad currently
    ...
    def _get_microsoft_records_to_sync(self, full_sync=False):
        """
        Return records that should be synced from emdad to Microsoft
    ...
    @api.model
    def _microsoft_to_emdad_values(
        self, microsoft_event: MicrosoftEvent, default_reminders=(), default_values=None, with_ids=False
    ):
        """
        Implements this method to return a dict of emdad values corresponding
        to the Microsoft event given as parameter
        :return: dict of emdad formatted values
        """
