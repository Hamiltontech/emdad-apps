...
import logging
import requests
from emdad.addons.microsoft_calendar.models.microsoft_sync import microsoft_calendar_token
...
from emdad.addons.microsoft_account.models.microsoft_service import DEFAULT_MICROSOFT_TOKEN_ENDPOINT
from emdad.addons.microsoft_calendar.utils.microsoft_calendar import InvalidSyncToken
...
class User(models.Model):
    ...
    def _sync_microsoft_calendar(self):
        ...
        # Microsoft -> emdad
        synced_events, synced_recurrences = self.env['calendar.event']._sync_microsoft2emdad(events) if events else (self.env['calendar.event'], self.env['calendar.recurrence'])

        # emdad -> Microsoft
        recurrences = self.env['calendar.recurrence']._get_microsoft_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences._sync_emdad2microsoft()
        ...
        (events - synced_events)._sync_emdad2microsoft()
        self.microsoft_last_sync_date = fields.datetime.now()
