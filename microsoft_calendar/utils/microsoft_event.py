...
from emdad.api import model
from typing import Iterator, Mapping
from collections import abc
from emdad.tools import ReadonlyDict
...
class MicrosoftEvent(abc.Set):
    ...
    Inspired by emdad recordset, one instance can be a single Microsoft event or a
    ...
    def emdad_id(self, env):
        return self._emdad_id

    def _meta_emdad_id(self, microsoft_guid):
        """Returns the emdad id stored in the Microsoft Event metadata.
    ...
    def emdad_ids(self):
        """
        Get the list of emdad event ids already mapped with Outlook events (self)
        """
        return tuple(e._emdad_id for e in self if e._emdad_id)

    def _load_emdad_ids_from_db(self, env, force_model=None):
        """
        Map Microsoft events to existing emdad events:
        1) extract unmapped events only,
        2) match emdad events and Outlook events which have both a ICalUId set,
        3) match remaining events,
        Returns the list of mapped events
        """
        mapped_events = [e.id for e in self if e._emdad_id]
        ...
        res = env.cr.fetchall()
        emdad_events_ids = [val[0] for val in res]
        emdad_events = model_env.browse(emdad_events_ids)

        # 1. try to match unmapped events with emdad events using their iCalUId
        unmapped_events_with_uids = unmapped_events.filter(lambda e: e.iCalUId)
        emdad_events_with_uids = emdad_events.filtered(lambda e: e.ms_universal_event_id)
        mapping = {e.ms_universal_event_id: e.id for e in emdad_events_with_uids}

        for ms_event in unmapped_events_with_uids:
            emdad_id = mapping.get(ms_event.iCalUId)
            if emdad_id:
                ms_event._events[ms_event.id]['_emdad_id'] = emdad_id
                mapped_events.append(ms_event.id)

        # 2. try to match unmapped events with emdad events using their id
        unmapped_events = self.filter(lambda e: e.id not in mapped_events)
        mapping = {e.ms_organizer_event_id: e for e in emdad_events}

        for ms_event in unmapped_events:
            emdad_event = mapping.get(ms_event.id)
            if emdad_event:
                ms_event._events[ms_event.id]['_emdad_id'] = emdad_event.id
                mapped_events.append(ms_event.id)

                # don't forget to also set the global event ID on the emdad event to ease
    ...
    def owner_id(self, env):
        ...
        1) the current emdad user is the organizer of the event according to Outlook event, so return his id.
        2) the current emdad user is NOT the organizer and:
           2.1) we are able to find a emdad user using the Outlook event organizer email address and we use his id,
           2.2) we are NOT able to find a emdad user matching the organizer email address and we return False, meaning
                that no emdad user will be able to modify this event. All modifications will be done from Outlook.
        """
        if self.isOrganizer:
            return env.user.id
        if self.organizer.get('emailAddress') and self.organizer.get('emailAddress').get('address'):
            # Warning: In Microsoft: 1 email = 1 user; but in emdad several users might have the same email
    ...
    def match_with_emdad_events(self, env) -> 'MicrosoftEvent':
        """
        Match Outlook events (self) with existing emdad events, and return the list of matched events
        """
        ...
        mapped_recurrences = recurrence_candidates._load_emdad_ids_from_db(env, force_model=env["calendar.recurrence"])

        # then, try to match events
        events_candidates = (self - mapped_recurrences).filter(lambda x: not x.is_recurrence())
        mapped_events = events_candidates._load_emdad_ids_from_db(env)
