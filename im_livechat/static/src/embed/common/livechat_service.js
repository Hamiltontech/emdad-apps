...
export const emdad_VERSION_KEY = `${location.origin.replace(
    /:\/{0,2}/g,
    "_"
)}_im_livechat.emdad_version`;
...
export class LivechatService {
    ...
    async initialize() {
        ...
        if (!this.options.isTestChatbot) {
            ...
            const prevemdadVersion = browser.localStorage.getItem(emdad_VERSION_KEY);
            const curremdadVersion = init?.emdad_version;
            const visitorUid = this.visitorUid || false;
            const userId = session.user_id || false;
            if (
                prevemdadVersion !== curremdadVersion ||
                (this.sessionCookie && visitorUid !== userId)
            ) {
                this.leaveSession({ notifyServer: false });
            }
            browser.localStorage.setItem(emdad_VERSION_KEY, curremdadVersion);
        }
