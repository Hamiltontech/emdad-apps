...
from emdad.addons.hw_drivers.websocket_client import WebsocketClient
...
class Manager(Thread):
    def send_alldevices(self, iot_client=None):
        """
        This method send IoT Box and devices informations to emdad database
        """
        server = helpers.get_emdad_server_url()
        if server:
            subject = helpers.read_file_first_line('emdad-subject.conf')
            if subject:
            ...
            _logger.warning('emdad server not set')

    def run(self):
        """
        Thread that will load interfaces and drivers and contact the emdad server with the updates
        """
        ...
        iot_client = WebsocketClient(helpers.get_emdad_server_url())
        ...
        if helpers.get_emdad_server_url():
            iot_client.start()
