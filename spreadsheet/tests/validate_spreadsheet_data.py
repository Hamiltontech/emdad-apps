...
import json
import re
from emdad.tests.common import TransactionCase, Form
from emdad.exceptions import ValidationError

markdown_link_regex = r"^\[([^\[]+)\]\((.+)\)$"

xml_id_url_prefix = "emdad://ir_menu_xml_id/"

emdad_view_link_prefix = "emdad://view/"


def emdad_charts(data):
    """return all emdad chart definition in the spreadsheet"""
    figures = []
    for sheet in data["sheets"]:
        figures += [
            dict(figure["data"], id=figure["id"])
            for figure in sheet["figures"]
            if figure["tag"] == "chart" and figure["data"]["type"].startswith("emdad_")
        ]
    return figures
...
def emdad_view_links(data):
    """return all view definitions embedded in link cells.
    urls looks like emdad://view/{... view data...}
    """
    return [
        json.loads(url[len(emdad_view_link_prefix):])
        for url in links_urls(data)
        if url.startswith(emdad_view_link_prefix)
    ]
...
def filter_fields(data):
    ...
    charts = emdad_charts(data)
    emdad_version = data.get("emdadVersion", 1)
    if emdad_version < 5:
...
def fields_in_spreadsheet(data):
    ...
    all_fields = chain(
        ...
        extract_fields(chart_fields, emdad_charts(data)).items(),
        extract_fields(emdad_view_fields, emdad_view_links(data)).items(),
...
def menus_xml_ids_in_spreadsheet(data):
    ...
    return set(data.get("chartemdadMenusReferences", {}).values()) | {
