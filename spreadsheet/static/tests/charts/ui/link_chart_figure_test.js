...
QUnit.module(
    ...
    () => {
        QUnit.test(
            "icon external link isn't on the chart when its not linked to an emdad menu",
            async function (assert) {
                ...
                await nextTick();
                const emdadMenu = model.getters.getChartemdadMenu(chartId);
                assert.equal(emdadMenu, undefined, "No menu linked with the chart");
        ...
        QUnit.test(
            "icon external link is on the chart when its linked to an emdad menu",
            async function (assert) {
                const model = await createModelWithDataSource({
                    serverData: this.serverData,
                });
                const fixture = await mountSpreadsheet(model);
                createBasicChart(model, chartId);
                model.dispatch("LINK_emdad_MENU_TO_CHART", {
                    chartId,
                    emdadMenuId: 1,
                });
                const chartMenu = model.getters.getChartemdadMenu(chartId);
                assert.equal(chartMenu.id, 1, "emdad menu is linked to chart");
                await nextTick();
                const externalRefIcon = fixture.querySelector(".o-chart-external-link");
        ...
        QUnit.test(
            "icon external link is not on the chart when its linked to a wrong emdad menu",
            async function (assert) {
                const model = await createModelWithDataSource({
                    serverData: this.serverData,
                });
                ...
                model.dispatch("LINK_emdad_MENU_TO_CHART", {
                    chartId,
                    emdadMenuId: "menu which does not exist",
                });
                const chartMenu = model.getters.getChartemdadMenu(chartId);
        ...
        QUnit.test(
            "click on icon external link on chart redirect to the emdad menu",
            async function (assert) {
                ...
                const chartMenu = model.getters.getChartemdadMenu(chartId);
                assert.equal(chartMenu.id, 2, "emdad menu is linked to chart");
                await nextTick();
        ...
        QUnit.test(
            "Click on chart in dashboard mode redirect to the emdad menu",
            async function (assert) {
                const doActionStep = "doAction";
                mockActionService(assert, doActionStep);
                const model = await createModelWithDataSource({
                ...
                assert.verifySteps(
                    [doActionStep],
                    "Clicking on a chart while on dashboard mode redirect to the emdad menu"
                );
            }
        );
        ...
        QUnit.test("can use menus xmlIds instead of menu ids", async function (assert) {
            ...
            model.dispatch("LINK_emdad_MENU_TO_CHART", {
                chartId,
                emdadMenuId: "documents_spreadsheet.test.menu2",
            });
            await nextTick();
        ...
        QUnit.test(
            ...
            async function (assert) {
                ...
                model.dispatch("LINK_emdad_MENU_TO_CHART", {
                    chartId,
                    emdadMenuId: "documents_spreadsheet.test.menu_without_action",
                });
                await nextTick();
