    ...
    () => {
        QUnit.test(
            ...
            async function (assert) {
                ...
                assert.equal(
                    exportedData.chartemdadMenusReferences[chartId],
                    1,
                    "Link to emdad menu is exported"
                );
                const importedModel = new Model(exportedData, { custom: { env } });
                const chartMenu = importedModel.getters.getChartemdadMenu(chartId);
                assert.equal(chartMenu.id, 1, "Link to emdad menu is imported");
            }
        );

        QUnit.test("Can undo-redo a LINK_emdad_MENU_TO_CHART", async function (assert) {
            const env = await makeTestEnv({ serverData: this.serverData });
            const model = new Model({}, { custom: { env } });
            createBasicChart(model, chartId);
            model.dispatch("LINK_emdad_MENU_TO_CHART", {
            ...
            assert.equal(model.getters.getChartemdadMenu(chartId).id, 1);
            model.dispatch("REQUEST_UNDO");
            assert.equal(model.getters.getChartemdadMenu(chartId), undefined);
