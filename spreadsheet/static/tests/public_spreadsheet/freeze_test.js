...
import { freezeemdadData } from "../../src/helpers/model";
...
QUnit.module("freezing spreadsheet", {}, function () {
    QUnit.test("emdad pivot functions are replaced with their value", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        assert.strictEqual(getCell(model, "A3").content, '=emdad.PIVOT.HEADER(1,"bar","false")');
        assert.strictEqual(
            getCell(model, "C3").content,
            '=emdad.PIVOT(1,"probability","bar","false","foo",2)'
        );
        assert.strictEqual(getEvaluatedCell(model, "A3").value, "No");
        assert.strictEqual(getEvaluatedCell(model, "C3").value, 15);
        const data = await freezeemdadData(model);
    ...
    QUnit.test("invalid expression with pivot function", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        setCellContent(model, "A1", "=emdad.PIVOT(1)+"); // invalid expression
    ...
    QUnit.test("emdad pivot functions detection is not case sensitive", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        setCellContent(model, "A1", '=emdad.pivot(1,"probability")');
        setCellContent(model, "A2", '=emdad.pivot(1,"probability")');
        const data = await freezeemdadData(model);
    ...
    QUnit.test("computed format is exported", async function (assert) {
        ...
        setCellContent(model, "A1", '=emdad.PIVOT(1,"pognon")');
    ...
    QUnit.test("emdad charts are replaced with an image", async function (assert) {
