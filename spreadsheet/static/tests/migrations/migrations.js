...
import { migrate, emdad_VERSION } from "@spreadsheet/o_spreadsheet/migration";
import { Model } from "@emdad/o-spreadsheet";

QUnit.module("spreadsheet > migrations");

QUnit.test("emdad formulas are migrated", (assert) => {
    const data = {
        sheets: [
            {
                cells: {
    ...
    assert.strictEqual(migratedData.sheets[0].cells.A1.content, `=emdad.PIVOT("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A2.content, `=emdad.PIVOT.HEADER("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A3.content, `=emdad.FILTER.VALUE("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A4.content, `=emdad.LIST("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A5.content, `=emdad.LIST.HEADER("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A6.content, `=emdad.PIVOT.POSITION("1")`);
    assert.strictEqual(migratedData.sheets[0].cells.A7.content, `=emdad.PIVOT("1")`);
});
...
QUnit.test("Pivot 'day' arguments are migrated", (assert) => {
    const data = {
        ...
        sheets: [
            {
                cells: {
                    A1: { content: `=emdad.PIVOT("1","21/07/2022")` },
                    A2: { content: `=emdad.PIVOT.HEADER("1","11/12/2022")` },
                    A3: { content: `=emdad.pivot("1","21/07/2021")` },
                    A4: { content: `=emdad.PIVOT("1","test")` },
                    A5: { content: `=emdad.pivot("1","21/07/2021")+"21/07/2021"` },
    ...
    const migratedData = migrate(data);
    assert.strictEqual(migratedData.sheets[0].cells.A1.content, `=emdad.PIVOT("1","07/21/2022")`);
    assert.strictEqual(
        migratedData.sheets[0].cells.A2.content,
        `=emdad.PIVOT.HEADER("1","12/11/2022")`
    );
    assert.strictEqual(migratedData.sheets[0].cells.A3.content, `=emdad.pivot("1","07/21/2021")`);
    assert.strictEqual(migratedData.sheets[0].cells.A4.content, `=emdad.PIVOT("1","test")`);
    assert.strictEqual(
        migratedData.sheets[0].cells.A5.content,
        `=emdad.pivot("1","07/21/2021")+"21/07/2021"`
    );
...
QUnit.test("emdad version is exported", (assert) => {
    const model = new Model();
    assert.strictEqual(model.exportData().emdadVersion, emdad_VERSION);
});
