...
import { Model, DispatchResult, helpers, tokenize } from "@emdad/o-spreadsheet";
import {
...
QUnit.module("spreadsheet > Global filters model", {}, () => {
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            const chartId = model.getters.getemdadChartIds()[0];
            fieldMatching = model.getters.getemdadChartFieldMatching(chartId, filterId);
            assert.equal(fieldMatching.chain, "created_on");
            assert.equal(fieldMatching.type, "date");
            assert.equal(fieldMatching.offset, undefined);
        }
    ...
    QUnit.test("emdad.FILTER.VALUE text filter", async function (assert) {
        assert.expect(3);

        const model = await createModelWithDataSource();
        setCellContent(model, "A10", `=emdad.FILTER.VALUE("Text Filter")`);
        await nextTick();
    ...
    QUnit.test("emdad.FILTER.VALUE date filter", async function (assert) {
        assert.expect(4);

        const model = await createModelWithDataSource();
        setCellContent(model, "A10", `=emdad.FILTER.VALUE("Date Filter")`);
        await nextTick();
        await addGlobalFilter(model, {
            id: "42",
            type: "date",
    ...
    QUnit.test("emdad.FILTER.VALUE relation filter", async function (assert) {
        assert.expect(6);
        const model = await createModelWithDataSource({
        ...
        setCellContent(model, "A10", `=emdad.FILTER.VALUE("Relation Filter")`);
        await nextTick();
        await addGlobalFilter(model, {
            id: "42",
            type: "relation",
    ...
    QUnit.test(
        "emdad.FILTER.VALUE formulas are updated when filter label is changed",
        async function (assert) {
            assert.expect(1);
            ...
            setCellContent(
                model,
                "A10",
                `=emdad.FILTER.VALUE("Cuillère") & emdad.FILTER.VALUE( "Cuillère" )`
            );
            const [filter] = model.getters.getGlobalFilters();
            const newFilter = {
                ...filter,
            ...
            assert.equal(
                ...
                `=emdad.FILTER.VALUE("Interprete") & emdad.FILTER.VALUE("Interprete")`
            );
        }
    );
                ...
                sheets: [
                    {
                        id: "sheet1",
                        cells: {
                            A1: { content: `=emdad.PIVOT("1", "probability")` },
                        },
                    },
                ],
                pivots: {
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            assert.verifySteps([]);
            setCellContent(model, "A1", `=emdad.PIVOT("1", "probability")`);
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            const dateFilters2 = getFiltersMatchingPivot(
                model,
                '=emdad.PIVOT.HEADER(1,"date:year","2016")'
            );
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            const filters = getFiltersMatchingPivot(
                model,
                '=emdad.PIVOT.HEADER(1,"date:month","08/2016","product_id","41")'
            );
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            setCellContent(model, "B3", '=emdad.PIVOT(1, "__count", "#product_id", 1)');
            await addGlobalFilter(
                model,
                {
                    id: "42",
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            setCellContent(model, "B3", '=emdad.PIVOT(1, "probability", "#product_id", 1)');
            await addGlobalFilter(
                model,
                {
                    id: "42",
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            setCellContent(model, "B3", '=emdad.PIVOT(1, "probability")');
            await addGlobalFilter(
                model,
                {
                    id: "42",
    ...
    QUnit.test("field matching is removed when list is deleted", async function (assert) {
        ...
        model.dispatch("REMOVE_emdad_LIST", { listId });
        assert.throws(
    ...
    QUnit.test("field matching is removed when an emdad chart is deleted", async function (assert) {
        ...
        assert.deepEqual(model.getters.getemdadChartFieldMatching(chartId, filter.id), matching);
        model.dispatch("DELETE_FIGURE", { id: chartId, sheetId });
        assert.throws(
            () => model.getters.getemdadChartFieldMatching(chartId, filter.id),
