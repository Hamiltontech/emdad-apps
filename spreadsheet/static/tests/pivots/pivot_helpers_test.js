...
import { constants, tokenize } from "@emdad/o-spreadsheet";
...
QUnit.module("spreadsheet > pivot_helpers", {}, () => {
    QUnit.test("Basic formula extractor", async function (assert) {
        const formula = `=emdad.PIVOT("1", "test") + emdad.LIST("2", "hello", "bla")`;
        const tokens = tokenize(formula);
        let functionName;
        let args;
        ({ functionName, args } = getFirstPivotFunction(tokens));
        assert.strictEqual(functionName, "emdad.PIVOT");
        ...
        assert.strictEqual(functionName, "emdad.LIST");
    ...
    QUnit.test("Extraction with two PIVOT formulas", async function (assert) {
        const formula = `=emdad.PIVOT("1", "test") + emdad.PIVOT("2", "hello", "bla")`;
        const tokens = tokenize(formula);
    ...
    QUnit.test("Number of formulas", async function (assert) {
        const formula = `=emdad.PIVOT("1", "test") + emdad.PIVOT("2", "hello", "bla") + emdad.LIST("1", "bla")`;
