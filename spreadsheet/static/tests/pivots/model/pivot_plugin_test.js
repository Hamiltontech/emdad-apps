...
QUnit.module("spreadsheet > pivot plugin", {}, () => {
    ...
    QUnit.test(
        ...
        async function (assert) {
            const { model } = await createSpreadsheetWithPivot();
            setCellContent(model, "C3", `=emdad.PIVOT(G10,"probability","bar","false","foo","2")+2`);
    ...
    QUnit.test("Can remove a pivot with undo after editing a cell", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        assert.ok(getCellContent(model, "B1").startsWith("=emdad.PIVOT.HEADER"));
    ...
    QUnit.test("rename pivot with empty name is refused", async (assert) => {
        const { model } = await createSpreadsheetWithPivot();
        const result = model.dispatch("RENAME_emdad_PIVOT", {
            pivotId: "1",
            name: "",
        });
    ...
    QUnit.test("Undo/Redo for RENAME_emdad_PIVOT", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        assert.equal(model.getters.getPivotName("1"), "Partner Pivot");
        model.dispatch("RENAME_emdad_PIVOT", { pivotId: "1", name: "test" });
    ...
    QUnit.test("Format header displays an error for non-existing field", async function (assert) {
        const { model } = await createSpreadsheetWithPivot();
        setCellContent(model, "G10", `=emdad.PIVOT.HEADER("1", "measure", "non-existing")`);
        setCellContent(model, "G11", `=emdad.PIVOT.HEADER("1", "non-existing", "bla")`);
        await nextTick();
            ...
            sheets: [
                {
                    ...
                    cells: {
                        A1: { content: `=emdad.PIVOT(1, "probability")` },
                        A2: { content: `=emdad.PIVOT(2, "probability")` },
                    },
                },
            ],
            pivots: {
            ...
            sheets: [
                {
                    id: "sheet1",
                    cells: {
                        A1: { content: `=emdad.PIVOT.HEADER(1, "measure", "probability")` },
                        A2: { content: `=emdad.PIVOT.HEADER(1, "product_id", 37)` },
                        A3: { content: `=emdad.PIVOT(1, "probability")` },
                    },
                },
            ],
            pivots: {
    ...
    QUnit.test("relational PIVOT.HEADER with missing id", async function (assert) {
        ...
        model.dispatch("UPDATE_CELL", {
            col: 4,
            row: 9,
            content: `=emdad.PIVOT.HEADER("1", "product_id", "1111111")`,
            sheetId,
        });
        await waitForDataSourcesLoaded(model);
        assert.equal(
    ...
    QUnit.test("relational PIVOT.HEADER with undefined id", async function (assert) {
        ...
        setCellContent(model, "F10", `=emdad.PIVOT.HEADER("1", "product_id", A25)`);
    ...
    QUnit.test("can import (export) contextual domain", async (assert) => {
        ...
        setCellContent(model, "A1", '=emdad.PIVOT(1, "probability")'); // load the data (and check the rpc domain)
        await nextTick();
        assert.strictEqual(
    ...
    QUnit.test("Can group by many2many field ", async (assert) => {
        ...
        assert.equal(getCellFormula(model, "A3"), '=emdad.PIVOT.HEADER(1,"tag_ids","false")');
        assert.equal(getCellFormula(model, "A4"), '=emdad.PIVOT.HEADER(1,"tag_ids",42)');
        assert.equal(getCellFormula(model, "A5"), '=emdad.PIVOT.HEADER(1,"tag_ids",67)');

        assert.equal(
            getCellFormula(model, "B3"),
            '=emdad.PIVOT(1,"probability","tag_ids","false","foo",1)'
        );
        assert.equal(
            getCellFormula(model, "B4"),
            '=emdad.PIVOT(1,"probability","tag_ids",42,"foo",1)'
        );
        assert.equal(
            getCellFormula(model, "B5"),
            '=emdad.PIVOT(1,"probability","tag_ids",67,"foo",1)'
        );

        assert.equal(
            getCellFormula(model, "C3"),
            '=emdad.PIVOT(1,"probability","tag_ids","false","foo",2)'
        );
        assert.equal(
            getCellFormula(model, "C4"),
            '=emdad.PIVOT(1,"probability","tag_ids",42,"foo",2)'
        );
        assert.equal(
            getCellFormula(model, "C5"),
            '=emdad.PIVOT(1,"probability","tag_ids",67,"foo",2)'
        );
    ...
    QUnit.test("PIVOT.HEADER grouped by date field without value", async function (assert) {
        for (const interval of ["day", "week", "month", "quarter", "year"]) {
            ...
            setCellContent(model, "A1", `=emdad.PIVOT.HEADER(1, "date:${interval}", "false")`);
    ...
    QUnit.test("can edit pivot domain", async (assert) => {
        ...
        model.dispatch("UPDATE_emdad_PIVOT_DOMAIN", {
            pivotId,
            domain: [["foo", "in", [55]]],
        });
