...
QUnit.test("click a web link", async (assert) => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: "[emdad](https://emdad.com)" } },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
    const cell = getEvaluatedCell(model, "A1");
    assert.strictEqual(urlRepresentation(cell.link, model.getters), "https://emdad.com");
    openLink(cell.link, env);
    assert.verifySteps(["https://emdad.com"]);
});
...
QUnit.test("click a menu link", async (assert) => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: "[label](emdad://ir_menu_xml_id/test_menu)" } },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
...
QUnit.test("click a menu link", async (assert) => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: `[a view](emdad://view/${JSON.stringify(view)})` } },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
    const cell = getEvaluatedCell(model, "A1");
    assert.strictEqual(urlRepresentation(cell.link, model.getters), "an emdad view");
    openLink(cell.link, env);
