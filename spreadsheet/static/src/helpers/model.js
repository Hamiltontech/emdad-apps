...
import { Model, parse, helpers, iterateAstNodes } from "@emdad/o-spreadsheet";
    ...
    for (const sheet of Object.values(data.sheets)) {
        ...
        for (const figure of sheet.figures) {
            if (figure.tag === "chart" && figure.data.type.startsWith("emdad_")) {
                ...
                const img = emdadChartToImage(model, figure);
                figure.tag = "image";
                figure.data = {
                    path: img,
 ...
 * @returns {boolean}
 */
function containsemdadFunction(content) {
    if (
        !content ||
        !content.startsWith("=") ||
        (!content.toUpperCase().includes("emdad.") && !content.toUpperCase().includes("_T"))
    ) {
        return false;
    }
    try {
        ...
        return iterateAstNodes(ast).some(
            (ast) =>
                ast.type === "FUNCALL" &&
                (ast.value.toUpperCase().startsWith("emdad.") ||
 ...
 * @returns {string}
 */
function emdadChartToImage(model, figure) {
