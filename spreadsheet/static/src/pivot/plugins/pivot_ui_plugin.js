...
export class PivotUIPlugin extends spreadsheet.UIPlugin {
    ...
    handleEvent(event) {
        ...
        switch (event.type) {
            case "ZonesSelected": {
                ...
                if (cell !== undefined && cell.content.startsWith("=emdad.PIVOT.HEADER(")) {
    ...
    handle(cmd) {
        switch (cmd.type) {
            ...
            case "REMOVE_PIVOT":
                ...
                break;
            case "REFRESH_PIVOT":
                this._refreshemdadPivot(cmd.id);
                break;
            case "REFRESH_ALL_DATA_SOURCES":
                this._refreshemdadPivots();
                break;
            ...
            case "UPDATE_emdad_PIVOT_DOMAIN": {
            ...
            case "REDO": {
                ...
                const domainEditionCommands = cmd.commands.filter(
                    (cmd) => cmd.type === "UPDATE_emdad_PIVOT_DOMAIN" || cmd.type === "INSERT_PIVOT"
                );
     ...
     *
     * =emdad.PIVOT(1,"expected_revenue","stage_id",2,"city","Brussels")
     * =emdad.PIVOT.HEADER(1,"stage_id",2,"city","Brussels")
     * =emdad.PIVOT.HEADER(1,"stage_id",2,"city","Brussels","measure","expected_revenue")
     *
     * the result is the same: ["stage_id", 2, "city", "Brussels"]
     *
     * If the cell is the result of emdad.PIVOT.TABLE, the result is the domain of the cell
    ...
    getPivotDomainArgsFromPosition(position) {
        ...
        if (functionName === "emdad.PIVOT.TABLE") {
        ...
        const domain = args.slice(functionName === "emdad.PIVOT" ? 2 : 1);
        if (domain.at(-2) === "measure") {
            return domain.slice(0, -2);
        }
        return domain;
    ...
    _refreshemdadPivots() {
        for (const pivotId of this.getters.getPivotIds()) {
            this._refreshemdadPivot(pivotId, false);
        }
    }
