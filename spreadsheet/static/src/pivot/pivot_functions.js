...
function getPivotCellValueAndFormat(pivotId, pivotCell) {
    if (!pivotCell.domain) {
        ...
        const fn = pivotCell.isHeader ? emdad_PIVOT_HEADER : emdad_PIVOT;
...
functionRegistry
    .add("emdad.FILTER.VALUE", emdad_FILTER_VALUE)
    .add("emdad.PIVOT", emdad_PIVOT)
    .add("emdad.PIVOT.HEADER", emdad_PIVOT_HEADER)
    .add("emdad.PIVOT.POSITION", emdad_PIVOT_POSITION)
    .add("emdad.PIVOT.TABLE", emdad_PIVOT_TABLE);
