...
patch(spreadsheet.components.FigureComponent.prototype, {
    ...
    async navigateToemdadMenu() {
        const menu = this.env.model.getters.getChartemdadMenu(this.props.figure.id);
        if (!menu) {
    ...
    get hasemdadMenu() {
        return this.env.model.getters.getChartemdadMenu(this.props.figure.id) !== undefined;
    },
    async onClick() {
        if (this.env.isDashboard() && this.hasemdadMenu) {
            this.navigateToemdadMenu();
        }
    },
});
