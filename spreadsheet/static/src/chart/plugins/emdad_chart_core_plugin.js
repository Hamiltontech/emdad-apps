...
export class emdadChartCorePlugin extends CorePlugin {
    constructor(config) {
        super(config);
        ...
        globalFiltersFieldMatchers["chart"] = {
            getIds: () => this.getters.getemdadChartIds(),
            getDisplayName: (chartId) => this.getters.getemdadChartDisplayName(chartId),
            getFieldMatching: (chartId, filterId) =>
                this.getemdadChartFieldMatching(chartId, filterId),
            getModel: (chartId) =>
    ...
    handle(cmd) {
        switch (cmd.type) {
            ...
            case "EDIT_GLOBAL_FILTER":
                if (cmd.chart) {
                    this._setemdadChartFieldMatching(cmd.filter.id, cmd.chart);
                }
                break;
        }
    }
     ...
     * @returns {string}
     */
    getemdadChartDisplayName(chartId) {
        return `(#${this.getemdadChartIds().indexOf(chartId) + 1}) ${
        ...
        for (const sheet of data.sheets) {
            if (sheet.figures) {
                for (const figure of sheet.figures) {
                    if (figure.tag === "chart" && figure.data.type.startsWith("emdad_")) {
                        this._addemdadChart(figure.id, figure.data.fieldMatching);
                    }
                }
            }
        }
    ...
    /**
     * Get the current emdadChartFieldMatching of a chart
     *
     * @param {string} chartId
     * @param {string} filterId
     */
    getemdadChartFieldMatching(chartId, filterId) {
        return this.charts[chartId].fieldMatching[filterId];
    }

    /**
     * Sets the current emdadChartFieldMatching of a chart
     *
     * @param {string} filterId
     * @param {Record<string,FieldMatching>} chartFieldMatches
     */
    _setemdadChartFieldMatching(filterId, chartFieldMatches) {
        const charts = { ...this.charts };
     ...
     */
    _addemdadChart(chartId, fieldMatching = undefined) {
        const charts = { ...this.charts };
        if (!fieldMatching) {
...
emdadChartCorePlugin.getters = [
    "getemdadChartIds",
    "getChartFieldMatch",
    "getemdadChartDisplayName",
    "getemdadChartFieldMatching",
];
