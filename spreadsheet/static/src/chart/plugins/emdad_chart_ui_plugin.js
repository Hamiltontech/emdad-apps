...
export class emdadChartUIPlugin extends UIPlugin {
    constructor(config) {
        super(config);
        ...
        globalFiltersFieldMatchers["chart"] = {
            ...
            waitForReady: () => this._getemdadChartsWaitForReady(),
    ...
    beforeHandle(cmd) {
        switch (cmd.type) {
            case "START":
                for (const chartId of this.getters.getemdadChartIds()) {
        ...
        switch (cmd.type) {
            ...
            case "REDO": {
                ...
                for (const cmd of domainEditionCommands) {
                    if (!this.getters.getemdadChartIds().includes(cmd.id)) {
                        continue;
                    }
                    const dataSource = this.getChartDataSource(cmd.id);
                    if (
            ...
            case "REFRESH_emdad_CHART":
                this._refreshemdadChart(cmd.chartId);
                break;
            case "REFRESH_ALL_DATA_SOURCES":
                this._refreshemdadCharts();
                break;
        }
    }
    ...
    getChartDataSource(chartId) {
        const dataSourceId = this._getemdadChartDataSourceId(chartId);
     ...
     * @return {Promise[]}
     */
    _getemdadChartsWaitForReady() {
        return this.getters
            .getemdadChartIds()
            .map((chartId) => this.getChartDataSource(chartId).loadMetadata());
    }

    _getemdadChartDataSourceId(chartId) {
    ...
    _refreshemdadCharts() {
        for (const chartId of this.getters.getemdadChartIds()) {
            this._refreshemdadChart(chartId);
        }
    }
}

emdadChartUIPlugin.getters = ["getChartDataSource"];
