...
export const emdad_VERSION = 6;

const MAP = {
    PIVOT: "emdad.PIVOT",
    "PIVOT.HEADER": "emdad.PIVOT.HEADER",
    "PIVOT.POSITION": "emdad.PIVOT.POSITION",
    "FILTER.VALUE": "emdad.FILTER.VALUE",
    LIST: "emdad.LIST",
    "LIST.HEADER": "emdad.LIST.HEADER",
};

const dmyRegex = /^([0|1|2|3][1-9])\/(0[1-9]|1[0-2])\/(\d{4})$/i;

export function migrate(data) {
    let _data = load(data, !!emdad.debug);
    const version = _data.emdadVersion || 0;
    if (version < 1) {
...
function migratePivotDaysParameters(formulaString) {
    ...
    const convertedAst = convertAstNodes(ast, "FUNCALL", (ast) => {
        if (["emdad.PIVOT", "emdad.PIVOT.HEADER"].includes(ast.value.toUpperCase())) {
...
export class emdadVersion extends CorePlugin {
    export(data) {
        data.emdadVersion = emdad_VERSION;
    }
}

emdadVersion.getters = [];

corePluginRegistry.add("emdadMigration", emdadVersion);
