...
export class ListUIPlugin extends spreadsheet.UIPlugin {
    ...
    handle(cmd) {
        switch (cmd.type) {
            case "START":
                ...
                break;
            case "INSERT_emdad_LIST": {
                const { id, linesNumber } = cmd;
                this._setupListDataSource(id, linesNumber);
                break;
            }
            case "SELECT_emdad_LIST":
                this._selectList(cmd.listId);
                break;
            case "REMOVE_emdad_LIST":
                if (cmd.listId === this.selectedListId) {
                    this.selectedListId = undefined;
                }
                break;
            case "REFRESH_emdad_LIST":
                this._refreshemdadList(cmd.listId);
                break;
            case "REFRESH_ALL_DATA_SOURCES":
                this._refreshemdadLists();
                break;
            ...
            case "REDO": {
                ...
                const domainEditionCommands = cmd.commands.filter(
                    (cmd) =>
                        cmd.type === "UPDATE_emdad_LIST_DOMAIN" || cmd.type === "INSERT_emdad_LIST"
                );
     ...
     *
     * @param {string} content emdad list formula
     */
