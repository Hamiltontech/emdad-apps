...
from emdad.tests.common import JsonRpcException, new_test_user, tagged
...
class TestTranslationController(HttpCaseWithUserDemo):
    ...
    def test_access_right(self):
        with self.assertRaises(JsonRpcException, msg="emdad.http.SessionExpiredException"):
        ...
        with self.assertRaises(JsonRpcException, msg="emdad.exceptions.AccessError"), mute_logger("emdad.http"):
            self._mock_translation_request({"message_id": self.message.id})
