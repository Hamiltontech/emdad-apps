...
export function usePartnerAutocomplete() {
    const keepLastemdad = new KeepLast();
    ...
    async function autocomplete(value) {
        ...
        let emdadSuggestions = [];
        let clearbitSuggestions = [];
        return new Promise((resolve, reject) => {
            const emdadPromise = getemdadSuggestions(value, isVAT).then((suggestions) => {
                emdadSuggestions = suggestions;
            });
            ...
            const concatResults = () => {
                // Add Clearbit result with emdad result (with unique domain)
                if (clearbitSuggestions && clearbitSuggestions.length) {
                    const websites = emdadSuggestions.map((suggestion) => {
                    ...
                    clearbitSuggestions.forEach((suggestion) => {
                        if (websites.indexOf(suggestion.domain) < 0) {
                            ...
                            emdadSuggestions.push(suggestion);
                        }
                    });
                }

                emdadSuggestions = emdadSuggestions.filter((suggestion) => {
                    return !suggestion.ignored;
                });
                emdadSuggestions.forEach((suggestion) => {
                    delete suggestion.ignored;
                });
                return resolve(emdadSuggestions);
            };

            whenAll([emdadPromise, clearbitPromise]).then(concatResults, concatResults);
        });
    }
    ...
    /**
     * Use emdad Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Promise}
     * @private
     */
    async function getemdadSuggestions(value, isVAT) {
        ...
        const suggestions = await keepLastemdad.add(prom);
