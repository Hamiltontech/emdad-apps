...
from emdad.addons.test_mail.tests.common import TestRecipients
from emdad.tests import tagged
from emdad.tools import mute_logger


@tagged("emdadbot")
class Testemdadbot(MailCommon, TestRecipients):

    @classmethod
    def setUpClass(cls):
        super(Testemdadbot, cls).setUpClass()
        cls.test_record = cls.env['mail.test.simple'].with_context(cls._test_context).create({'name': 'Test', 'email_from': 'ignasse@example.com'})

        cls.emdadbot = cls.env.ref("base.partner_root")
        ...
        cls.emdadbot_ping_body = '<a href="http://emdad.com/web#model=res.partner&amp;id=%s" class="o_mail_redirect" data-oe-id="%s" data-oe-model="res.partner" target="_blank">@emdadBot</a>' % (cls.emdadbot.id, cls.emdadbot.id)
        cls.test_record_employe = cls.test_record.with_user(cls.user_employee)

    @mute_logger('emdad.addons.mail.models.mail_mail')
    def test_fetch_listener(self):
        channel = self.user_employee.with_user(self.user_employee)._init_emdadbot()
        emdadbot = self.env.ref("base.partner_root")
        emdadbot_in_fetch_listeners = self.env['discuss.channel.member'].search([('channel_id', '=', channel.id), ('partner_id', '=', emdadbot.id)])
        self.assertEqual(len(emdadbot_in_fetch_listeners), 1, 'emdadbot should appear only once in channel_fetch_listeners')

    @mute_logger('emdad.addons.mail.models.mail_mail')
    def test_emdadbot_ping(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': self.emdadbot_ping_body, 'partner_ids': [self.emdadbot.id, self.user_admin.partner_id.id]})
        ...
        with patch('random.choice', lambda x: x[0]):
            self.assertNextMessage(
                ...
                sender=self.emdadbot,
                answer=False
            )
        # emdadbot should not be a follower but user_employee and user_admin should
        follower = self.test_record.message_follower_ids.mapped('partner_id')
        self.assertNotIn(self.emdadbot, follower)
    ...
    def test_onboarding_flow(self):
        ...
        self.assertNextMessage(
            last_message,  # no message will be post with command help, use last emdadbot message instead
            sender=self.emdadbot,
            answer=("@emdadBot",)
        )
        kwargs['body'] = ''
        ...
        channel.message_post(**kwargs)
        self.assertNotEqual(self.user_employee.emdadbot_state, 'onboarding_attachement')

        # Test miscellaneous messages
        self.user_employee.emdadbot_state = "idle"
        kwargs['partner_ids'] = []
    ...
    @mute_logger('emdad.addons.mail.models.mail_mail')
    def test_emdadbot_no_default_answer(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': "I'm not talking to @emdadbot right now", 'partner_ids': []})
        self.assertNextMessage(
