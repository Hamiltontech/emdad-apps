...
from emdad import api, Command, fields, models, tools, SUPERUSER_ID, _, _lt
from emdad.addons.rating.models import rating_data
from emdad.addons.web_editor.tools import handle_history_divergence
from emdad.exceptions import UserError, ValidationError, AccessError
from emdad.osv import expression
from emdad.tools.misc import get_lang
from emdad.addons.resource.models.utils import filter_domain_leaf
...
class Task(models.Model):
    ...
    def _notify_by_email_get_headers(self, headers=None):
        ...
        if self.project_id:
            current_objects = [h for h in headers.get('X-emdad-Objects', '').split(',') if h]
            current_objects.insert(0, 'project.project-%s, ' % self.project_id.id)
            headers['X-emdad-Objects'] = ','.join(current_objects)
        if self.tag_ids:
            headers['X-emdad-Tags'] = ','.join(self.tag_ids.mapped('name'))
        return headers
